...
|------|-----------|-------------|-----------|
| ~~**G1**~~ ✅ **GESCHLOSSEN 2026-08-26** | **D3 = 24 Monate** gesetzt (Owner-Sign-off Christian), D1/D4/D5/D6 mit-bestätigt (D2 = CO/technisch, bereits per Consensus entschieden, nicht Teil der Sign-off-Tabelle), **ADR-045 `Proposed`→`Accepted`**. Begründung der Zahl: **Zeremonie-Probe-Erzwingung (D6)**, nicht Kompromittierungs-Fenster-Kompensation. Beleg: `TL-14a-G1-decision-brief.md` §5 | ~~Christian~~ **erledigt** | ✅ **Runbook-Volltext + Zeremonie-Skripte entriegelt** |
...
| **G3** ⚠️ **neu bewertet 2026-08-26** | **Cross-Vendor-Pass**: `codex`/`agy` sind **VORHANDEN** — in `~/.local/bin/` (agy Binary, codex Symlink) —, nur **nicht im `PATH`**. Deshalb scheitert `pal:consensus`/`pal:codereview` mit *"executable not found in PATH"*, während der **direkte Aufruf über den absoluten Pfad funktioniert** (2026-08-26 belegt: echtes agy-Review von PR #353). ⇒ **PATH-Problem, kein Verfügbarkeitsproblem.** Für **Reviews ab sofort umgehbar**; für `pal`-getriebene **Consensus**-Läufe weiterhin blockiert, bis der PATH des MCP-Servers stimmt | Infra/Christian | Cross-Vendor-**Review** ✅ nutzbar · Cross-Vendor-**Consensus** weiter offen |
