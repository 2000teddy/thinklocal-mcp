...
- [ ] 🔴 **Identität auf libp2p-PeerID umstellen** — Konsens (2 pal:consensus-Läufe, einstimmig Option 1),
  ...
  **Umsetzung (Schritt 1 erledigt — Commit `1683396`, s. CHANGES 2026-06-03):**
  ...
  - [ ] **Follow-up (nicht-blockierend):** (a) Mesh-Peer-Eintrag bei Card-Re-Fetch auf die kanonische agentId umschlüsseln (heute funktional via PeerID-Auflösung gelöst, nur kosmetisch); (b) ✅ **erledigt (v0.34.52, PR 127b):** token-onboarded TLS-Bundle wird beim Laden fail-closed gegen `ca.crt.pem` validiert (`tls.ts` `loadOrCreateTlsBundle`, `certKeyMatches && verifyPeerCert` + 6 Regressionstests); (c) dedizierter mTLS-Integrationstest `issuerCertificate.fingerprint256 === certFingerprint(ca.crt.pem)` (live bereits bewiesen).
