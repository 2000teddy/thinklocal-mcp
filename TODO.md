...
- [ ] 🟠 **ADR-024-Rollout-Gate (= „Identität voll canonical", vor Re-Enroll .56/.222/.94)** — die 2 CR/PC-MEDIUMs aus #165: (a) CA-Gültigkeit `notBefore/notAfter` fail-closed im Retention-Verify + Test — ✅ **geschlossen (v0.34.20, `verifyPeerCert` caValid + Tests)**; (b) Trust-Distribution-Lifecycle retained fremd-Certs + Test — ✅ **geschlossen (v0.34.20, `selectTrustDistributionCa` fail-closed + Tests)**. **Offen:** PR-Merge (Christian-Gate), **dann** Re-Enroll → 100% canonical-emit. KEIN Re-Enroll/Deploy ohne Christians Wort.
