...
- [ ] 🔴 **.55 OUTBOUND host-seitig blockiert (peers_online=0) — Christian, sudo** — Node/libuv-`connectx` (macOS) → EHOSTUNREACH von 10.10.10.55 zu ALLEN Peers, während `curl`(`connect(2)`) 200 liefert; raw `net.connect` scheitert ebenfalls → **kein Daemon-Bug** (Trust-Bundle = 69bc0bc korrekt, alles per curl erreichbar). Routing sauber (1 Default-Route en10), WLAN aus → stale `connectx`/Kernel-Zustand. **Fix host-seitig: en10 bounce / `route flush` / Reboot von .55.** Bis dahin: .55 INBOUND ok, aber kein Outbound-Dial (peers_online=0 aus .55-Sicht; registry_sync mit .55 braucht .55-Outbound). Beweis: `node net.connect` EHOSTUNREACH vs curl 200, 2026-06-11.
- [ ] 🟠 **emit_canonical_sender Default true (v0.34.10, DRAFT-PR)** — committed-Default war false → canonical-Nodes fielen beim `git pull` auf Legacy zurück (TH01/.55). Fix: config.ts DEFAULTS + config/daemon.toml committed = true; sicher dank Fail-safe-Interlock. Branch `agent/claude-code/emit-canonical-default-true`.
- [ ] 🟡 **.55 canonical-Cutover-Abschluss (mit Christian)** — nach .55-Host-Reset (Outbound ok): Linux-Cleanup (5 Restarts → stale legacy `host/813bdd16` droppen) + .94-Restart (Dublette) → canonical `count=6` + registry_sync fleet-weit. Runbook: `docs/REENROLL-55-RUNBOOK.md`.

- [ ] 🟠 **Static-Peer Online-Self-Healing (v0.34.9, DRAFT-PR)** — ein static_peer der transient offline flappt (dual-homed macOS .55) blieb dauerhaft offline (Reconciler one-shot bei mDNS-an + `checkPeers` schließt offline-Peers vom /health-Re-Poll aus). Fix: `resolveStaticReconcileSteadyMs` → Steady-Reconcile IMMER (mdns-unabhängig); `addPeer` feuert `onPeerOnline` beim Offline→Online-Re-Connect. Branch `agent/claude-code/static-peer-steady-always`. Macht .55 self-healing **unabhängig** vom Host-Routing-Fix. Voraussetzung für mDNS-Auto-Rediscovery von .55 bleibt Christians Host-Routing-Fix (en10 primäre Default-Route, sudo); mit static_peer + diesem Fix ist .55 aber AUCH ohne Host-Fix voll im Mesh.
- [ ] 🟡 **registry_sync-Coordinator libp2p-Bug (pre-existing, fleet-weit)** — `/api/status` zeigt `last_error:"multiaddrs[0].getPeerId is not a function"`, `converged:false` für Peers auf mehreren Linux-Nodes. Pre-existing (auch auf 92e6058), NICHT von ADR-026/0.34.9. Caps propagieren trotzdem via Gossip/SKILL_ANNOUNCE. Eigene Diagnose/Fix nötig (Multiaddr→PeerID-Extraktion im registry-sync-libp2p-adapter).
- [ ] 🟠 **ADR-026 Symmetrische Auth-Peer-Discovery (DRAFT-PR, ready — Orchestrator merged mit `gh --admin`)** — fixt 403 „Unknown sender" auf `SKILL_ANNOUNCE` für nicht-selbst-entdeckte authentifizierte Peers (mobil/NAT/Cross-Subnet/`mdns_enabled=false`). Authentifizierte mTLS-Inbound-Verbindung lernt den Peer → ephemere AUTHN-only `authenticatedSeen`-Map (TTL 15min, LRU 256), von `resolvePeerPublicKey` vor 403 konsultiert. **INVARIANTE (CR HIGH 1):** `authenticated_unapproved` leakt NICHT in AUTHZ — `mesh.isApprovedPeerSender` gatet `REGISTRY_SYNC`+`SKILL_ANNOUNCE` (this.peers-only). Config `auto_register_authenticated_peers` (Default an). Branch `agent/claude-code/symmetric-auth-discovery`. CO/CR(gpt-5.5, 2 HIGH+1 MED+2 LOW gefixt)/PC, 983+6 Tests grün. **#164/#166 unangetastet.** Nach Merge: Fleet-Deploy + `.55`-Test durch Orchestrator. Offene Frage (separat, Christian): darf ein `authenticated_unapproved`-Peer seine EIGENEN Caps advertisen? — aktuell NEIN (strikt). Verwandt: `th02-phase3-flip-blocker` (Flip-Re-Fetch separat offen).
- [ ] 🟠 **ADR-025 .55-Mesh-Join (DRAFT-PR, wartet auf Review)** — dual-homed macOS .55 joint nicht: Daemon-Start vergiftet `connectx` transient, der frühere Einmal-static_peer-Connect traf das Fenster → 0 Peers. Fix (3 additiv, config-gegatet): (1) `discovery.mdns_enabled=false` → kein Bonjour + kein libp2p-mDNS (echtes static-only); (2) Static-Peer-Reconciler (sofort + 15s/5min, static-only 60s steady) statt Einmal-Burst; (3) `discovery.preferred_interfaces` (wired vor WiFi) → erlaubt `/16`. Branch `agent/claude-code/static-discovery-join`. CO/CR(gpt-5.5)/PC grün, 0 CRITICAL/HIGH, 962+6 Tests. **KEIN Deploy/Merge ohne Christians Wort; Test auf .55 durch Orchestrator.** .55-Empfehlung nach Merge: `mdns_enabled=false` + static_peers.
