...
- [x] 🔴 **.55 EHOSTUNREACH — mDNS-Interface-Pin vergiftet macOS connectx-scoped-routing** (Befund + Fix 2026-06-08, **v0.34.5**) — Auf dem dual-homed macOS-Node .55 (en10 = Mesh + zweite Default-Route-NIC) brach die **blosse Anwesenheit** des Daemons das connectx-scoped-routing **prozessweit** (10.10.10/24 → REJECT, EHOSTUNREACH für ALLE Connects, auch plain `node`). Root-Cause: der Daemon ruft kein `route`/`IP_BOUND_IF` — die **einzige** Interface-Scoping-Op ist der mDNS-Pin `bonjour-service {interface: meshIp}` → `setMulticastInterface`. (#162-Escape-Hatch half NICHT, sitzt im Outbound-Connect statt im mDNS-Socket.) **Fix:** Opt-out-Flag `disable_mdns_interface_pin` (Default false; `TLMCP_DISABLE_MDNS_INTERFACE_PIN=1`) → Bonjour ohne `interface` (nur `bind:0.0.0.0`), A-Record-Hygiene + Fail-Closed bleiben aktiv. Siehe ADR-019 „.55 connectx-Vergiftung". **Offen:** Live-Verifikation (.55 Route-Heal + Daemon mit/ohne Flag) durch Orchestrator (lokal, sudo); Empfehlung Pin-Disable IMMER mit `allowed_mesh_cidrs` + `static_peers`.
...
- [ ] 🔴 **Identität auf libp2p-PeerID umstellen** — Konsens (2 pal:consensus-Läufe, einstimmig Option 1),
  ...
  **Umsetzung (Schritt 1 erledigt — Commit `1683396`, s. CHANGES 2026-06-03):**
  ...
  - [ ] **OFFEN (Ops, GATE):** Produktiv-Flotten-Flip (.56/.52/.222) — NUR auf Christians ausdrückliches Wort. Reihenfolge: erst ALLE Nachbarn auf v0.34.2 (empfangsseitige Härtung), DANN per-Node-Flip + Noise-Re-Handshake + Gegenprobe; danach `TLMCP_STRICT_IDENTITY=1` + Legacy-Pfad entfernen.
