...
- [~] **[v5.1] TL-14a (≈3 h)** CA-Zweistufen-Umzug: Runbook (Offline-Wurzel-Zeremonie, Intermediate TH01,
  ...
  - **⛔ G2-RATIFIZIERUNG NÖTIG (Christian, 2026-08-25):** `docs/architecture/TL-14a-consensus-result-C.md` —
    der **CO-Anteil von G2 ist erledigt** (adversarialer `pal:consensus`-Doppellauf). **Ergebnis: C wird
    gesplittet** — **C1** (lokales `isRevoked`-Enforcement, App-Ebene, `agent-card.ts:311`+`:353`, bestehende
    `crl.ts` verdrahten) = **blockierend vor TL-14b, aber klein**; **C2** (Mesh-Verteilung) = **Fast-Follow,
    bewusst nichts bauen**. Form: **Denylist**, kein CRL/OCSP. Stärkster Grund für C1: die kalte TH02-Reserve
    (D6) ist ohne Sperrfähigkeit nur **halb** aktivierbar (Verfügbarkeit ja, Integrität nein). **Offen: die
    Owner-Hälfte** (Ratifizierungs-Tabelle im Dokument) + Freigabe des C1-Slices; ADR-045 §74/§100 werden erst
    **danach** nachgezogen. Rückwirkung auf G1: kürzere D3-Laufzeit ist **kein** Revocation-Ersatz, Empfehlung
    **24 Mon.** als Zeremonie-Probe-Erzwingung (Korridor 1–3 J unverändert). Same-Vendor-Panel — **G3 erneut
    verifiziert offen** (`codex`/`agy` nicht aufrufbar). Slice **nicht** vorgezogen.
