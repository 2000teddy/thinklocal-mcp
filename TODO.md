...
- [x] **#165 ADR-024 Canonical-Cert-Retention** (357842f) — re-enrolltes canonical Cert wird behalten. ⚠️ Rest-MEDIUMs offen → „ADR-024-Rollout-Gate" unten.
...
- [ ] 🟠 **ADR-024-Rollout-Gate (= „Identität voll canonical", vor Re-Enroll .56/.222/.94)** — die 2 CR/PC-MEDIUMs aus #165 verifizieren/schließen: (a) CA-Gültigkeit `notBefore/notAfter` fail-closed im Retention-Verify + Test; (b) Trust-Distribution-Lifecycle retained fremd-Certs + Test. **Dann** Re-Enroll → 100% canonical-emit. KEIN Re-Enroll/Deploy ohne Christians Wort.

### Offen — Backlog (Design-First, nächste Sprints)
- [ ] 🟡 **registry_sync-Coordinator `getPeerId`-Bug + Capability-Count-Drift** — `/api/status`: `multiaddrs[0].getPeerId is not a function`, converged:false (fleet-weit, pre-existing). Caps propagieren via Gossip trotzdem. Diagnose+Fix im registry-sync-libp2p-adapter (Multiaddr→PeerID). (5-Tage-Plan B7.)
