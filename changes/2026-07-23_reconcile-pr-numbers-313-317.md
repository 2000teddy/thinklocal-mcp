...
- **TODO.md (4 Einträge):** `Verdrahtungs-Hook` → `, #315`; `CR-LOW-Härtung` → `, #316` (die vorhandene
  ...
  (Kanal-Bindungs-Primitive)` → `, #317`. Höchste TODO-PR-Ref rückt **#312 → #317**.
...
- Spaltenzahl der reconcilten Zeilen = Referenz-Zeile #312 (11 Pipes); die #313-Zeile hat 12, weil ihr
  eigener Fließtext einen **pre-existing escaped** `\|` in einem Code-Span zitiert — rendert korrekt.
- Nach dem Reconcile: **0** Zeilen mit `(offen, base=main)` außer der Zeile dieses PRs.
- Diff rein 1:1 in-place (14 Annotationen) + additive Selbst-Doku; keine Struktur-/Inhaltsänderung.
...
- **CR:** externe Review-of-Record am PR (Doc-only) + Self-CR — jede PR↔Zeile-Zuordnung gegen den gemergten
  PR-Titel/-Timestamp gegengeprüft, keine erfundene Nummer.
