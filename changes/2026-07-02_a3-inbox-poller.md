...
- **`buildDaemonInboxDeps` / `createDaemonInboxPoller`** — Daemon-I/O gegen `requestDaemon`
  (`GET /api/inbox?unread=true[&for_instance=<component>]`, `POST /api/inbox/mark-read`).
- **`for_instance`-Vertrag korrekt angeglichen (Review-Blocker):** die Inbox-API (ADR-005,
  `validateInstanceParam` → `SPIFFE_COMPONENT_REGEX`) erwartet die **Instanz-Komponente `<id>`**, und
  `to_agent_instance` speichert `getAgentInstance(to)` (= genau diese Komponente). Der Poller nahm die
  **volle** A1-Instanz-URI → hätte live **400** ergeben. Fix: `instanceComponentForQuery()` extrahiert
  die 4. Komponente aus der vollen URI (bzw. reicht eine bereits nackte Komponente durch), bevor
  `for_instance` gesetzt wird. Damit ist der Client-Vertrag exakt konsistent zum Server/ADR-005.
