...
export function registerInboxApi(server: FastifyInstance, deps: InboxApiDeps): void {
  ...
  server.post('/api/inbox/send', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // ---- BROADCAST PATH ----
    // If the target ends with /instance/*, fan out to all active instances
    // on this daemon. Used for announcements and system events.
    if (body.to.endsWith('/instance/*') && agentRegistry) {
      ...
      for (const inst of instances) {
        const targetPayload: AgentMessagePayload = {
          ...payload,
          message_id: randomUUID(), // unique ID per recipient
    ...
    try {
      ...
      // ACK-Signaturpruefung: Peer-PublicKey aus AgentCard lookup + Envelope decode.
      // Falls der Peer keinen PublicKey hat oder die Signatur ungueltig ist,
      // loggen wir eine Warnung aber akzeptieren die Zustellung trotzdem
      // (HTTP 2xx ist das primaere Delivery-Signal).
          ...
          log?.info(
            {
              to: body.to,
              message_id: messageId,
              ack_status: ackPayload.status,
              ack_verified: true,
            },
            'AGENT_MESSAGE gesendet + ACK verifiziert',
          );
        } else {
          log?.info(
            { to: body.to, message_id: messageId, subject: body.subject ?? null },
            'AGENT_MESSAGE gesendet (ACK nicht verifiziert — kein Peer-PublicKey)',
          );
        }
      } catch (ackErr) {
        log?.warn(
          ...
          'AGENT_MESSAGE gesendet aber ACK-Signatur ungueltig',
        );
      }
