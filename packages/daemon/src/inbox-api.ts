...
export function registerInboxApi(server: FastifyInstance, deps: InboxApiDeps): void {
  ...
  server.post('/api/inbox/send', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (body.to.endsWith('/instance/*') && agentRegistry) {
      ...
      log?.info({ to: body.to, fanout: results.length }, 'AGENT_MESSAGE broadcast');
      eventBus?.emit('inbox:new', { from: ownAgentId, message_id: messageId, to: body.to, broadcast: true, fanout: results.length });
    ...
    try {
      ...
      try {
        ...
        if (peerPublicKey && ackBody.length > 0) {
          ...
          log?.info({ to: body.to, message_id: messageId, ack_verified: true, ack_status: ackPayload.status }, 'AGENT_MESSAGE + ACK verifiziert');
        } else {
          log?.info({ to: body.to, message_id: messageId }, 'AGENT_MESSAGE gesendet (kein Peer-PublicKey)');
        }
      } catch (ackErr) {
