    ...
    try {
      ...
      try {
        ...
        if (peerPublicKey && ackBody.length > 0) {
          ...
          log?.info(
            { to: body.to, message_id: messageId, ack_status: ackPayload.status, ack_verified: true },
        ...
        log?.warn(
          { to: body.to, message_id: messageId, err: ackErr instanceof Error ? ackErr.message : String(ackErr) },
