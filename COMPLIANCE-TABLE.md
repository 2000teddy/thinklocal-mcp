...
| 111 | tbd       | ADR-005 Per-Agent-Inbox Phase 1 (SPIFFE 4-Komponenten + Schema-Migration) | 04-09 21:30 | —  | —  | ✅ | ✅ | ✅ | ✅ | **Dieser PR.** `spiffe-uri.ts` (27 Tests, zentraler SPIFFE_COMPONENT_REGEX) + `agent-inbox.ts` Schema-Migration v1→v2 mit `createSchemaV2`/`migrateToV2` Split + `inbox-api.ts` Loopback-Fix (normalizeAgentId fuer GPT-5.4-Gotcha) + `for_instance`/`include_legacy` Query-Params + SECURITY.md Threat-Model fuer Application-Layer-Routing. 61/61 Tests gruen (27 SPIFFE + 12 ADR-005 Inbox + 8 Fastify + 14 back-compat). CR Gemini Pro security focus: 0 HIGH/CRITICAL, 2× MEDIUM (Instance-ID Validation Asymmetrie, Schema-Init Klarheit) + 1× LOW (DRY Helper) alle gefixt mit 8 Injection-Regression-Tests. PC Gemini Pro: 1× HIGH (Regex-Literal im DRY-Helper statt Import) mid-fix gefixt. CO entfaellt (Konsensus aus PR #84). |
