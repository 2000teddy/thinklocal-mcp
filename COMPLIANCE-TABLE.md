...
*Letzte Aktualisierung: 2026-07-21 08:10 — docs(arch): ADR-046 Wire-Feature/Version-Exchange (Scoping, Proposed); scopet den TL-12-Slice-C-Prerequisite (Agent-Card `protocol`-Block + fail-closed `peerSupportsFeature`, Seed-Flag `order-envelope-v2`) — Consumer-Seite existiert (mesh hält Card), nur annonciertes Feld + version-compat-Verdrahtung fehlen; Impl CO-gated; doc-only, kein Code/Deploy/Secret.*
