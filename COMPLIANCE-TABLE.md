...
| 108 | tbd       | Socket-Pool-Fix fuer MCP-Stdio (Bug-Fix aus PR #86 Live-Test)    | 04-09 17:46 | —  | —  | ✅ | ✅ | ✅ | ✅ | **Dieser PR** — Root-Cause aus PR #86 Live-Test: `local-daemon-client.ts` erzeugte pro Call neuen HttpsAgent ohne keepAlive → Socket-Pool-Exhaustion nach ~4 h → `socket hang up`. Fix: globaler Agent-Cache pro dataDir mit mtime-Fingerprint-Invalidierung + graceful shutdown handlers in `mcp-stdio.ts` + 128+signal Exit-Codes. 5 neue Regression-Tests, 5/5 gruen. CR Gemini Pro (0 HIGH/CRITICAL, 1× MEDIUM + 3× LOW gefixt). PC Gemini Pro (1× CRITICAL via `pal:challenge` als False-Positive bestaetigt — Funktion ist vollstaendig synchron, atomisch im Node Event-Loop; defensiver Kommentar eingebaut. 1× HIGH Exit-Code gefixt). CO/CG entfallen (reiner Bug-Fix, siehe CLAUDE.md). |
