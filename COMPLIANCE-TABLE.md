...
| #283 | (offen, base=main) | 2026-07-17 13:05 | n/a | n/a | ✅ | ⚠️ | ✅ | ✅ | **Test-Infra (cert-fixture Slice) → CO/CG entfallen** (kein Runtime-Change, keine Design-Frage; leitet mTLS-/Loopback-Semantik aus gemergtem Code + agent-card.ts ab). Zieht die zwei offenen `it.todo` des Wire-Scaffolds an → `tl11-wake-wire.conformance.test.ts` **11 grün** (0 todo). **§2 mTLS-Pflicht:** zweiter Harness mit demselben Vertrag wie der cardServer (Fastify `https`+`requestCert`+`rejectUnauthorized`, agent-card.ts:229-230), In-Memory-CA/Server-/Client-Leaf (node-forge), undici-`WebSocket`+`Agent`-Client → gültiges Client-Cert erreicht `/ws` (`system:connected`), cert-los + `ws://` → TLS-Reset. **§2 Nicht-Loopback → `4003`:** Bindung an echte Nicht-Loopback-IPv4 (kein `trustProxy` → `req.ip`=Socket-Peer), `it.skipIf` auf Loopback-Hosts. **TS:** Datei 11 grün, Suite **1746 grün** (127 Files), tsc(strict)/neue-Datei-Lint 0. **CR ⚠️:** die genannten externen Reviewer **codex/agy sind nicht im PATH** (`[[pal-review-backend-agy-missing]]`) → adversariales **Claude-Review-Subagent** (NICHT MiniMax/pal:chat) — **kein HIGH**; M1 (Timeout-Sentinel → Negatives beweisen echten Reset), M2 (Abgrenzung mTLS-Semantik ≠ Prod-Verdrahtung; cardServer-Wiring-Test = Follow-up), L2 (Listener-Race), L3 (Link-Local-Ausschluss) adressiert. **CR-Backend-Ersetzung offen zur User-Bestätigung.** PC: Secret-Scan clean. Kein Merge (Christian-gated + CR-Backend-Frage). |
...
*Letzte Aktualisierung: 2026-07-17 13:05 — test(tl11): §2 mTLS-Pflicht + Nicht-Loopback-4003 cert-fixture Slice (#283); Suite 1746 grün; CR via Claude-Subagent (codex/agy nicht im PATH).*
