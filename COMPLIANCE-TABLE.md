...
*Letzte Aktualisierung: 2026-07-21 09:45 — feat(mcp,api): TL-21 Slice 5 Task-Skelett (`GET /api/tasks/overview` + MCP `list_tasks_overview`); reiner gemeinsamer Envelope-Builder `buildTaskOverview` (Signale statt Blobs + `by_state`-Histogramm, Invariante Summe===count), same-source `getAllTasks()`, total gegen malformed Felder; +25 Tests, Suite 1856 grün; CR 1 MEDIUM (Doc-Drift) gefixt, kein HIGH; read-only/additiv, kein Deploy/Secret.*
