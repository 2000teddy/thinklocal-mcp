...
**Typ:** Additiver Read-View (TL-21 Peer-Skelett — read-only, kein State, `index.ts` unangetastet). Kein Deploy/Secret. **DO:** `packages/daemon/src/peer-skeleton.ts`, `packages/daemon/src/peer-skeleton.test.ts`, `packages/daemon/src/dashboard-api.ts`, `packages/daemon/src/dashboard-api.test.ts`, `docs/architecture/TL-21-skeleton-disclosure.md`, `changes/2026-07-20_tl21-peers-overview.md`, `CHANGES.md`, `TODO.md`, dieser Eintrag.

---

*Letzte Aktualisierung: 2026-07-20 14:12 — feat(api): TL-21 Peer-Skelett-Auskunft (`GET /api/peers/overview`); additive read-only Projektion, total gegen malformed Card-Daten, keine Korrektheits-Bugs im CR; index.ts unangetastet.*
