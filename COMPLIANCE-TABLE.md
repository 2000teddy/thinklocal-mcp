...
| #323 | (base=main, gemergt) | 2026-07-23 13:50 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Additive ungegatete reine Primitive (TL-12 `canonicalOrderKeyId` — format-stabiler Keyid über DER-SPKI) + eingefalteter #322-Reconcile → CO/CG entfallen** (kein Design-Diff, **keine** offene Entscheidung berührt; die Kanonisierung ist im Slice-B-Scoping §3 bereits als Anforderung festgehalten und wird hier nur als reine Primitive erfüllt — Prep-Form wie #314/#317/#322; `clink`/`gemini` nicht im PATH). **Abgrenzung vorweg — ausdrücklich NICHT Slice B/B0:** keine der vier §9-Christian-Entscheidungen wird angerührt (`[orders] execute`-Opt-in, Epoch-Grenze `T`, ausführbare Startmenge, Revocation-Autorität); es entsteht **kein** Executable-Profil, **keine** Config, **kein** Schema, **kein** Ledger, **keine** Denylist, **kein** Execute-Pfad. **Der Defekt — reproduziert, nicht vermutet:** `orderKeyId` hasht die PEM-**Textdarstellung** (`createHash('sha256').update(publicKeyPem)`), der Keyid ist damit eine Eigenschaft der **Serialisierung** statt des Schlüsselmaterials — dasselbe Schlüsselpaar mit CRLF + zwei Leerzeilen ergibt einen **anderen** Keyid, während die DER-SPKI-Bytes **byte-identisch** sind. Heute folgenlos (nur Anzeige/Audit, `index.ts:876`); sobald der Keyid die `UNIQUE(signer_keyid, order_nonce)`-Spalte des Idempotenz-Ledgers (B1) **oder** den Join-Key der Revocation-Denylist (B2b) bildet, ist er ein **Umgehungspfad**: ein Relay macht denselben Auftrag durch bloßes **Umformatieren** des mitgelieferten PEM erneut ausführbar (neue Ledger-Zeile) bzw. umgeht eine Sperre (andere Denylist-Zeile) — **ohne** die Signatur zu berühren, die über die verbatim Bytes läuft. Genau deshalb verlangt das Scoping §3 die Kanonisierung **bevor** der Keyid diese Rolle bekommt. **Neu `canonicalOrderKeyId(publicKeyPem): string | null`:** sha256hex über die **DER-SPKI-Bytes** — gleichwertige PEM-Kodierungen ⇒ derselbe Keyid, verschiedene Schlüssel bleiben unterscheidbar, algorithmus-agnostisch (P-256 **und** Ed25519 geprüft); **fail-closed** ⇒ `null` bei nicht parsebarer Eingabe, **kein** Ersatz-Keyid und **kein** Fallback auf den PEM-Hash (ein geratener Keyid wäre genau die Kollision, die die Funktion verhindern soll); wirft nie. **Privates Schlüsselmaterial wird ausdrücklich abgelehnt** — `createPublicKey()` leitet daraus klaglos den öffentlichen Schlüssel ab, ein versehentlich übergebener Signier-Schlüssel bekäme sonst einen *gültig aussehenden* Keyid und Geheimmaterial liefe durch die Funktion (beim Testen aufgefallen, nicht nachträglich behauptet). **`orderKeyId` bleibt unverändert** (es stempelt bereits gespeicherte Zeilen — ein Wechsel wäre eine **Datenmigration** und gehört in den gateten B0-Slice); sein Doc-Kommentar benennt die Malleabilität jetzt explizit. **0 Aufrufer** (per grep belegt) ⇒ kein Runtime-Change. **TS:** +8 Tests (`signed-order.test.ts`: **Defekt-Beleg**, Stabilität gegen dieselbe Umformatierung, verschiedene Schlüssel bleiben verschieden, Ed25519, 8 unbrauchbare Eingaben ⇒ `null`, **privater Schlüssel ⇒ `null`**, wirft nie, plus **Regressionsanker „`orderKeyId` unverändert"** gegen einen stillen Semantikwechsel gespeicherter Zeilen), Suite **2000 grün** (141 Files), tsc(strict) 0, geänderte Dateien eslint 0/0, prettier clean. **CR:** externes Review am PR (adversariales Claude-Subagent). **PC:** Secret-Scan clean (Tests erzeugen Schlüssel in-memory, kein Material im Repo). **Eingefaltet:** #322-Reconcile (COMPLIANCE-Nummer + CHANGES-Überschrift + TODO-Eintrag, `gh`-verifiziert `mergedAt=2026-07-23T11:45:36Z`/`17937b3`). **Unverändert gated:** Slice B (§9), Slice C (V1–V3), ADR-046-Producer (CO), TL-11 Slice B (Host-Hop), TL-10-Verdrahtung (D1-Loader/D3). Kein State/Deploy/Secret, Risiko-Delta **null**. |

**Typ:** Additive ungegatete reine Primitive (TL-12 kanonischer Order-Keyid — 0 Aufrufer, kein Runtime-Change, kein Gate) + #322-Reconcile. Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/signed-order.ts`(+test), `docs/architecture/TL-12-slice-b-execution-scoping.md` §3, `changes/2026-07-23_tl12-canonical-order-keyid.md`, `CHANGES.md`, `TODO.md`, dieser Eintrag.

| #324 | (base=main, gemergt) | 2026-07-23 14:50 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Additive ungegatete reine Primitive (TL-12 B1-Prep: Reserve-vor-Dispatch/Commit-Vertrag als Zustandsmaschine) + die von §4 beauftragte Spezifikation → CO/CG entfallen** (keine neue Architektur-Frage: `TL-12-slice-b-execution-scoping.md` §4 **beauftragt** wörtlich „Reserve/Commit-Protokoll **jetzt** gemeinsam mit B3s Dispatch-Kontrakt spezifizieren — sonst wird B1 blind gegen B3 gebaut und muss neu geschrieben werden“; die Zustandsmenge folgt zwingend aus der dort festgelegten at-most-once-Semantik; Prep-Form wie #314/#317/#322/#323; `clink`/`gemini` nicht im PATH). **Abgrenzung — das ist NICHT B1/B3:** es entsteht **keine** Persistenz (keine Tabelle, kein `UNIQUE`-Index, keine Transaktions-Klammer), **kein** Execute-Pfad, **kein** Dispatch, **keine** TTL-Prüfung, **keine** Denylist, **kein** Rate-Fence; **keine** der vier §9-Christian-Entscheidungen wird berührt (`[orders] execute`-Opt-in/D-OWNER, Epoch-Grenze `T`/D-EPOCH, ausführbare Startmenge, Revocation-Autorität). Das Protokoll sagt **nicht**, *ob* ausgeführt werden darf — nur, dass eine erlaubte Ausführung **höchstens einmal** stattfindet. Neu `order-ledger-protocol.ts`: `nextLedgerState(current, event) → LedgerTransition` + Guards `mayDispatch`/`isFinal`, **rein und total** (kein I/O, keine Uhr, kein Zufall; unbekannter Zustand/Event ⇒ `malformed` statt Wurf oder Rateversuch). **Die Zustandsmenge ist erzwungen, nicht gewählt:** aus „Reserve **vor** Dispatch“ + „at-most-once: Crash-nach-Claim wird **nie** ausgeführt“ folgt `reserved` → `committed`|`failed`, **beide terminal**; ein zweiter `reserve` auf eine bekannte Nonce ist der **semantische Zwilling** des `UNIQUE (signer_keyid, order_nonce)`-Constraints und wird abgelehnt — **auch auf `failed`**, denn ein gemeldeter Fehlschlag kann ein **Timeout** sein, dessen Nebenwirkung bereits eingetreten ist (ein Retry wäre at-**least**-once, genau das schließt §4 aus). `mayDispatch` ist der **EINZIGE** erlaubte Auswertungspfad (analog `isApproved`/TL-09 und `isRoutable`/TL-10) und nur beim erfolgreichen `reserve` wahr ⇒ der Dispatch hängt strukturell an genau **einem** Übergang. **Doku (neues §4.1):** Übergangstabelle + **beide** Seiten, die §4 zusammen sehen will — was **B1** persistieren muss (`UNIQUE` als *technischer* Zwilling der Zustandsprüfung, die sie **nicht** ersetzt sondern fachlich doppelt; Claim-`INSERT` committet **vor** dem Dispatch; `signer_keyid` **muss** der kanonische DER-SPKI-Keyid aus **#323** sein, sonst wäre die `UNIQUE`-Spalte durch bloßes PEM-Umformatieren umgehbar; `messages` ist **nicht** die Idempotenz-Einheit, `idx_messages_order` bewusst nicht UNIQUE; `replayGuard` bleibt ungeeignet) und was **B3** einhalten muss (nur bei `mayDispatch` dispatchen, danach **genau ein** `commit`/`fail`, und `mayDispatch === false` ist **kein Fehler**, sondern der Normalfall bei Duplikaten/Crash-Wiederanlauf — **nicht** in einen Retry umdeuten). **TS:** +27 Tests (Happy Path; `duplicate-claim` auf `reserved`/`committed`/**`failed`**; **Crash-nach-Claim bleibt undispatched**; ganze Ereignisfolge gibt `mayDispatch` **genau einmal** frei; `not-reserved`; doppeltes/gekreuztes `commit`/`fail` ⇒ `already-final`; 7 unbekannte Events + 5 unbekannte Zustände ⇒ `malformed`; wirft nie; **vollständige 12-Kombinationen-Matrix** ⇒ exakt **eine** gibt einen Dispatch frei; plus die 9 CR-Regressionen), Suite **2027 grün** (142 Files), tsc(strict) 0, neue Dateien eslint 0/0, prettier clean. **CR ✅:** adversariales Claude-Subagent (`agy`/`codex` nicht im PATH, `[[pal-review-backend-agy-missing]]`) mit dem Auftrag, einen zweiten Dispatch zu konstruieren — **kein HIGH** (BFS über alle Ereignisfolgen bis Tiefe 7, 27 feindliche Zustands- × 29 Event-Werte inkl. `Object.create(null)`, Proxy, `valueOf`/`toString`/`Symbol.toPrimitive`, Prototype-Pollution ⇒ **max. 1 Dispatch pro Lauf, nie aus einem Nicht-`null`-Zustand, kein Wurf**), aber **3 MEDIUM + 4 LOW — alle an der Wurzel behoben**: **(M1, echter Defekt)** der `malformed`-Zweig setzte `state: null` — und `null` ist genau das Sentinel für „Zeile existiert nicht ⇒ `reserve` erlaubt"; ein Aufrufer, der dem dokumentierten `state = t.ok ? t.next : t.state` folgt, konnte damit eine **bereits beanspruchte Nonce wieder claimbar waschen** (vom Reviewer demonstriert). Fix: das Feld heißt jetzt `observed`, ist **rein diagnostisch**, wird **nur** bei gültigem Zustand gesetzt und trägt nie einen Ersatzwert — es gibt schlicht kein Feld mehr, aus dem ein `null` zurückgelesen werden könnte (+9 Tests, davon einer exakt der demonstrierte Pfad). **(M2)** „kann strukturell nicht zweimal freigegeben werden" gilt **nicht** für die Race zweier Prozesse — das Modul ist zustandslos; `mayDispatch` ist **notwendig, nicht hinreichend**, Autorität ist der committete Claim-`INSERT`. In Modul-Doc und §4.1-B3-Regel 1 klargestellt (Reihenfolge: prüfen → claimen → dispatchen). **(M3)** §4.1 verlangte den kanonischen Keyid, warnte aber nicht, dass `messages.signer_keyid` **heute schon** den malleablen `orderKeyId`-Wert hält (`index.ts:876`) — ein `INSERT … SELECT` aus dieser Spalte hätte die #323-Umgehung wiederhergestellt; jetzt explizit „aus `signer_pubkey` **neu berechnen**, nie kopieren" plus die fehlende `null`-Regel (kein Keyid ⇒ kein Claim ⇒ kein Dispatch). **LOW:** „erzwungen" auf das ehrliche Maß zurückgenommen (at-most-once ist gepinnt, der Kollaps auf **einen** `failed`-Zustand ist eine **bewusst konservative Wahl** — pre-effect- bzw. in-doubt-Ausgang wären gleichwertige Alternativen; Preis: „nie versucht" ist nicht von „möglicherweise gelaufen" unterscheidbar); §4-Zitat auf den Crash-nach-Claim-Fall zurückgeführt, den es tatsächlich betrifft; der **legitime Wiederanlauf über eine NEUE Nonce** ist jetzt benannt (Punkt 3 verbietet den Retry, nicht die Neuausstellung); für **verwaiste `reserved`-Zeilen** ist eine Audit-/Read-Surface-Pflicht ergänzt (ein stiller, dauerhafter No-op ist der schlechteste Betriebsausgang); Ergebnisse sind jetzt `Object.freeze`d und `mayDispatch` prüft zusätzlich `next` (Schranke statt Konvention). **PC:** Secret-Scan clean (kein I/O, keine Credentials). **Eingefaltet:** #323-Reconcile (`gh`-verifiziert `mergedAt=2026-07-23T12:45:51Z`/`95cc6fe`, `reviewDecision=APPROVED`). **Unverändert gated:** B1-**Persistenz**, B2a/B2b, B3-Ausführung, alle vier §9-Entscheidungen, Slice C (V1–V3), ADR-046-Producer (CO), TL-11 Slice B (Host-Hop), TL-10-Verdrahtung (D1-Loader/D3). Kein State/Deploy/Secret, Risiko-Delta **null**. |

**Typ:** Additive ungegatete reine Primitive (TL-12 B1-Prep Reserve/Commit-Protokoll — 0 Aufrufer, keine Persistenz, kein Execute-Pfad) + §4-beauftragte Spezifikation + #323-Reconcile. Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/order-ledger-protocol.ts`(+test), `docs/architecture/TL-12-slice-b-execution-scoping.md` §4.1, `changes/2026-07-23_tl12-b1-ledger-protocol.md`, `CHANGES.md`, `TODO.md`, dieser Eintrag.

| #325 | (base=main, gemergt) | 2026-07-23 15:30 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Erdung des TL-12-Prereq-Pfads (ADR-046 Rev. 2: Implementierungs-Anker, fail-closed-Grenzen, Seed-Flag, CO-Grenze) → CO/CG/TS entfallen** (kein Code/Test-Diff; das Update **trifft keine** der offenen Entscheidungen, es verifiziert Anker und hält Grenzen fest, die ohnehin gelten; `clink`/`gemini` nicht im PATH). **Abgrenzung:** **kein** `protocol`-Block auf der Card, **kein** Producer-Code, **kein** ORDER-Handler, **kein** Sender-Flip — die CO-pflichtigen Beschlüsse (Platzierung; Vokabular/`protocol_version`-Semver) bleiben unberührt offen. **Konkreter Befund — drei Anker der Erstfassung (#308) waren verschoben** und gegen `e994e65` korrigiert: Card-Fetch + Identitäts-Check + Store `index.ts:1491-1502` → **`:1530-1541`**; zweiter Card-Consume-Pfad `:1553` → **`:1592`/`:1603`**; und der für Slice-C-Vorbehalt **V1** entscheidende `default`-Drop `:932-934` → **`:936-938`** — gerade dieser dritte trägt die Beleglast des Slice-C-Parks („top-level ORDER fällt still in den default-Drop"), ein falscher Anker dafür entwertet die Argumentation. **Bestätigt korrekt geblieben:** `agent-card.ts:22-111` (weiterhin ohne `protocol`/`features`), `mesh.ts:20/189/258`, `pinned-card-fetch.ts:35`, `version-compat.ts` (außerhalb Tests weiterhin ohne Aufrufer). **Neu gepinnt:** der Producer war nur namentlich genannt → `agent-card.ts:480` `buildCard()`. **Neu benannt:** ein **dritter**, in der Erstfassung fehlender Card-Consume-Pfad (`index.ts:720`, Fetch `:631`/`:731`) — wer den `protocol`-Block liest, muss prüfen, ob **alle** Pfade denselben Identitäts-Check durchlaufen, bevor eine Card als Feature-Quelle gilt. **Neue Sektionen:** **§5** Anker-Tabellen (Producer/Consumer/Drift); **§6** fail-closed-Grenzen, die **unabhängig vom CO** gelten (kein `protocol`-Block · `features` fehlt/kein Array/leer · nicht gelistet · Card nicht abrufbar · **Identitäts-Check nicht bestanden** ⇒ je `false`; bereits kodifiziert in `wire-feature.ts` #314, und ein Producer-Slice **darf die Semantik nicht aufweichen**, insbesondere nicht „Feature-Liste fehlt ⇒ aus `protocol_version` ableiten“) plus die ausdrückliche Feststellung **Feature-Advertisement ist kein Trust-Grant** (Pairing, Approval-Gates, Slice-B-Allowlist bleiben unberührt); **§7** Seed-Flag `order-envelope-v2` — **Empfänger**-Semantik („ich kann X entgegennehmen“, nicht „ich sende X“), und ein Node darf es **erst** setzen, wenn sein Dispatch top-level ORDER wirklich behandelt, sonst ist das Flag eine **Lüge**, die beim Sender genau den V1-Drop auslöst; es sagt **nichts** über Ausführung (Slice B), Signatur-Vertrauen, TTL oder Denylist; **§8** was CO-gated bleibt (Platzierung, Vokabular/Semver, Producer-Befüllung, Empfänger-Handler, Sender-Flip) — mit dem Hinweis, dass der CO an einen **Owner-/Infra-Schritt** gebunden ist (`pal:consensus` PATH-blockiert, `[[pal-review-backend-agy-missing]]`), nicht an weitere Repo-Arbeit. **TS:** kein `.ts`-Diff, Suite unverändert **2027 grün** (142 Files). **CR ✅ (echtes externes Review, `agy` 1.1.5):** `agy` und `codex` sind **doch verfügbar** — nicht im `PATH`, aber als Binaries unter `~/.local/bin` direkt aufrufbar. Damit ist die in früheren Einträgen geführte Notiz „`agy`/`codex` nicht im PATH“ zwar wörtlich richtig, aber als *Verfügbarkeits*-Aussage falsch gewesen. `codex` scheitert weiterhin (Token abgelaufen, `codex login` ist ein interaktiver Owner-Schritt); **`agy --print` lief durch und lieferte 2 HIGH + 1 MEDIUM — alle behoben:** **HIGH-1:** §6 war **nicht** CO-unabhängig, obwohl sie es behauptete — die Tabellenzeilen nannten konkret einen `protocol`-Block, ein Feld `features` und `protocol_version` und nahmen damit **genau die beiden CO-Fragen vorweg** (Platzierung; Vokabular). Neu gefasst: die Tabelle spricht jetzt von der „**annoncierten Feature-Liste**" — wo immer der CO sie verortet — und die Ableitungs-Warnung nennt keine konkreten Versionsfelder mehr. **HIGH-2:** eine **vierte** Card-Parse-Stelle war übersehen (`pinned-card-fetch.ts:63`). Nachgetragen mit der Nuance, die der Reviewer nicht hatte: sie parst die Card bewusst **nur** auf `{spiffeUri, publicKey}` und gibt die volle Card **nicht** zurück — also kein Feature-Konsument, aber genau deshalb eine Falle für einen Folge-Slice, der „die Card lesen" will; daraus folgt eine **neue §6-Zeile**: liefert ein Pfad nicht die volle Card ⇒ `false` („fehlt" heißt dort „nicht gelesen", nicht „unterstützt nicht"). **MEDIUM:** der Anker `1530-1541` war als „Fetch + Identitäts-Check + Store" etikettiert, deckt aber nur Cast + Check + Store — der `fetch` beginnt bei `:1524`; Anker auf **`1524-1541`** präzisiert. Zusätzlich klargestellt, dass der Seed-Name `order-envelope-v2` ein **Vorschlag, kein Beschluss** ist. **PC:** Secret-Scan clean (nur Doku). **Eingefaltet:** #324-Reconcile (`gh`-verifiziert `mergedAt=2026-07-23T13:18:44Z`/`e994e65`). Kein Code/State/Deploy/Secret, Risiko-Delta **null**. |

**Typ:** Doc-only Erdung (ADR-046 Rev. 2 — Anker gegen HEAD verifiziert, fail-closed-Grenzen, Seed-Flag-Semantik, CO-Grenze) + #324-Reconcile. Kein Deploy/Secret/Host. **DO:** `docs/architecture/ADR-046-wire-feature-version-exchange.md`, `changes/2026-07-23_adr046-impl-anchors.md`, `TODO.md`, `CHANGES.md`, dieser Eintrag.

| #326 | (base=main, gemergt) | 2026-07-23 16:10 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Additive Daemon-Verdrahtung hinter Env-Flag mit Default AUS (TL-11 Reconciliation-Sweep) → CO/CG entfallen** (alle Entscheidungen liegen **innerhalb** der in ADR-047 §3 dokumentierten Optionen — keine neue Architektur-Frage; `clink`/`gemini` nicht im PATH). **Einordnung vorweg:** der TL-11-„MVP" im engeren Sinn (registrierte Agenten, Weckruf-Ereignis, Pfad zum Mesh-Postfach) ist **bereits gemergt** — Slice A/ADR-043 (#271), gerichteter Wake (#277), Consumer-Contract + Runbook, Draht-Conformance (#282/#283), Emitter-Ende-zu-Ende (#320), Sweep-Kern (#322). Dieser Slice schließt die **verbliebene echte Lücke** statt Vorhandenes zu wiederholen. **Die Lücke:** `agent:wake` ist best-effort/lossy (ADR-043 §3) — war der Supervisor einer Instanz beim Eintreffen der Nachricht gerade weg (Neustart/Reconnect/Crash), ist das Wake **verloren** und die Post liegt still im Postfach, bis der Konsument von sich aus pollt. Neu `sweep-wiring.ts`: `runReconciliationSweep` (ein Lauf, wirft nie) + `registerReconciliationSweep` (Hook + Abmelde-Funktion), in `index.ts` **+20/-0 additiv** hinter `TLMCP_WAKE_SWEEP_ENABLED=1` verdrahtet und im Shutdown abgemeldet. **Die vier ADR-047-§3-Punkte innerhalb der dort aufgeführten Optionen entschieden:** (1) **ergänzt** statt verschiebt — die Konsumenten-Cold-Start-Pflicht bleibt, der Daemon sieht nicht jeden Supervisor-Neustart; (2) Auslöser **`agentRegistry.on('register')`**, **zielgerichtet auf die registrierende Instanz** — der Hook **existiert bereits**, kein neuer Timer und **kein** Eingriff in die sicherheitsgehärtete WS-Datei; (3) **eigener `WakeCoalescer`** (§3 Option 1) — rate-begrenzt **und** nicht vom Inbox-Verkehr geschluckt, sonst verpuffte der Sweep genau im Fenster, das er beheben soll (§5 liest sich damit als „≤1 Wake pro Instanz pro Fenster **je Quelle**"; die **Idempotent**-Zeile der Spec deckt Zusatz-Wakes ohnehin ab); (4) **Flag, Default AUS** (Regime wie TL-09b) ⇒ ohne Flag wird die Verdrahtung gar nicht erst aufgerufen, **kein Verhaltens-Delta**. **Nur `register` löst aus** — `unregister`/`stale` heißt, die Instanz ist weg; sie zu wecken wäre sinnlos und im `stale`-Fall ein Wake an einen toten Konsumenten. **Bewusst offen gelassen:** die **Legacy-Politik** — `unreadCount({forInstance})` zählt per Default keine `to_agent_instance IS NULL`-Zeilen; der Sweep **erbt** diesen Default, statt ihn still zu verschieben (das wäre eine Vertragsfrage), benannt im Modul-Doc und in ADR-047 §3. **Fail-safe** (Nachbesserung, kein kritischer Pfad): werfende Registry ⇒ Sweep übersprungen · werfender Bus ⇒ die übrigen Instanzen bekommen ihr Wake trotzdem · werfender Zähler ⇒ Instanz entfällt (#322) · der Listener läuft **synchron** im Registry-Pfad und ist dort zusätzlich gekapselt. **Fail-closed:** ohne routbare SPIFFE kein Wake. **TS:** +18 Tests (`sweep-wiring.test.ts`: gerichtetes Wake mit SPIFFE · kein Wake ohne Post/ohne SPIFFE · **Wake ist inhaltsfrei** (die Anzahl ungelesener Nachrichten reist NICHT mit — Metadaten-Leak) · Coalescing im Fenster + nach Ablauf · **Sweep-Coalescer getrennt vom Emitter-Coalescer** · 3 Fail-safe-Fälle · nur `register` löst aus · Abmelde-Funktion · **Integration gegen die echte `AgentRegistry`**), Suite **2045 grün** (143 Files), tsc(strict) 0, neue Dateien eslint 0/0, prettier clean; `index.ts` **nicht** ganz-reformatiert. **CR ✅ — `agy` 1.1.5: 1 HIGH an der Wurzel behoben, Rest GREEN.** **HIGH (Performance-Hazard):** der Hook ignorierte die registrierende Instanz und fegte bei **jedem** `register` die **ganze** Registry — der Listener läuft **synchron** im `register()`-Pfad und `unreadFor` ist eine synchrone SQLite-Abfrage, also **M × N** Abfragen bei M gleichzeitigen Reconnects (O(N²)) ausgerechnet beim Massen-Neustart. Behoben durch `sweepInstance()`: **genau eine** Abfrage pro Registrierung — zugleich **semantisch präziser**, weil das Ereignis „dieser Supervisor ist wieder da“ sagt, nicht „alle sind da“; der flächendeckende `runReconciliationSweep` bleibt exportiert, aber **nicht** im Hot Path. +5 Regressionstests (Zähler wird **genau einmal** gefragt; nur die registrierende Instanz wird geweckt; unbekannte Instanz-ID wirft nicht). **GREEN bestätigt:** kein falsches Wake-Ziel (SPIFFE fest aus `agentRegistry.list()`, fail-closed ohne SPIFFE), **kein Metadaten-Leak** (`unreadCount` bleibt im Daemon), „Default AUS“ **lückenlos** (Verdrahtung exklusiv im Flag-`if`, Shutdown-Abmeldung erreicht), regulärer Emitter-Pfad ungestört (Zusatz-Wakes deckt die **Idempotent**-Zeile ab), Wurf-Sicherheit auf allen Pfaden, beide Doku-Aussagen (Consumer-Contract §7.3, ADR-047 §3) korrekt. **PC:** Secret-Scan clean. **Doku:** ADR-047 §3 Umsetzungsstand (welche Option je Punkt und **warum**) + §4-Tabelle auf „erledigt"; `TL-11-wake-consumer-contract.md` **§7.3 (neu)** — für den Konsumenten ändert sich **nichts** (gleiche Wake-Form, gleiche gerichtete Zustellung, Sweep-Wake ununterscheidbar), und die **Cold-Start-Pflicht bleibt bestehen**. **Eingefaltet:** #325-Reconcile (`gh`-verifiziert `mergedAt=2026-07-23T13:43:54Z`/`bd2fe98`). **Unverändert gated:** TL-11 Slice B (Host-Hop), der **Flag-Flip** in einer laufenden Instanz (owner), WS-Instanz-Bindung + Opt-in-Broadcast (ADR-047 §1/§2). Kein Deploy/Secret, Risiko-Delta **null** ohne Flag. |

**Typ:** Additive Daemon-Verdrahtung hinter Env-Flag, Default AUS (TL-11 Reconciliation-Sweep) + #325-Reconcile. Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/sweep-wiring.ts`(+test), `packages/daemon/src/index.ts` (+20/-0), `docs/architecture/ADR-047-tl11-wake-followups-scoping.md` §3/§4, `docs/architecture/TL-11-wake-consumer-contract.md` §7.3, `changes/2026-07-23_tl11-sweep-wiring.md`, `CHANGES.md`, `TODO.md`, dieser Eintrag.

| #327 | (base=main, gemergt) | 2026-07-23 16:45 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Referenz (TL-12 Zustellpfad: signierter Auftrag → Postfach → Abarbeitung) → CO/CG/TS entfallen** (kein Code/Test-Diff, keine Entscheidung, kein Gate verschoben; `clink`/`gemini` nicht im PATH). **Warum:** die TL-12-Doku ist über vier Dateien verteilt und jeweils **slice-zentriert**; es fehlte die **durchgehende** Sicht — welche Station heute wirklich existiert, was sie garantiert, wo der gebaute Pfad aufhört, und **an welcher Station genau** die offenen Gates aus TL-08/09/10 sitzen. **Neu `TL-12-delivery-path.md`:** S1 signieren (`signed-order.ts:57,69`, Body-Marker statt eigenem `MessageType` — das IST V1) · S2 Transport (bestehender Mesh-Pfad, TL-12 fügt **keinen** hinzu) · S3 Ingest (`index.ts:870` `classifyInboundOrder`: Verify gegen den **Transport**-Pubkey + `issuer === envelope.sender` ⇒ Relay-Schutz; **Tri-State**, also nie ein stiller Downgrade zu Plain, sondern `INVALID` + `ORDER_VERIFY_FAILED`-Audit `:882`) · S4 Postfach (`agent-inbox.ts:256` `store()` nimmt nur `OrderContext | null` ⇒ `is_order` **typsystemisch unfälschbar**; `signed_bytes` verbatim + `signer_pubkey` immutable ⇒ später **re-verifizierbar**; `ORDER_RX` `:905-909`) · S5 Read-Surface (`inbox-api.ts:399` re-verifiziert **live** je Zeile via `verifyStoredOrder`, fail-closed und wirft nie ⇒ eine bösartige Zeile legt die Liste nicht lahm) · **S6 Abarbeitung: ⛔ NICHT gebaut** (kein Executor/Ledger/Denylist/Rate-Fence — „Signatur ≠ Ausführungs-Erlaubnis", CO einstimmig). **TL-08/09/10 ausdrücklich verortet statt weggelassen:** eine eigene Tabelle sagt, an welcher Station jedes Gate greift — §9 (vier Owner-Entscheidungen) **vor** S6; TL-09b `resolveApproval` (`mcp-ingress.ts:174`) und TL-10 (Freigabe-Matrix, D1-Loader + **D3-Sign-off** offen) am schreibenden MCP-Aufruf; TL-08 (Gate-Flip sensitive→Redaction) am Werkzeug selbst. **Konsequenz explizit:** selbst bei entschiedenem Slice B liefe ein ausgeführter Auftrag **zusätzlich** durch TL-09b/TL-10 und TL-08 — der Auftrag ist ein *Antrag*, kein Freibrief, und **keines** dieser Gates wird durch die Auftrags-Signatur ersetzt. **Ende des Pfades belegt** (V1 default-Drop `index.ts:936-938` · V2 `version-compat` ohne Aufrufer · V3 `store()`-Kopplung an `AgentMessagePayload`) samt der Feststellung, dass der ehrliche nächste Baustein **ADR-046** ist, nicht Slice C. **Für S6 bereits vorbereitet und benannt:** `canonicalOrderKeyId` (#323) und `order-ledger-protocol.ts` (#324), beide 0 Aufrufer. **Alle zehn Code-Anker einzeln gegen `c4b5261` verifiziert** (Skript-Abgleich Zeile↔Symbol, nicht aus dem Gedächtnis zitiert). **TS:** kein `.ts`-Diff, Suite unverändert **2045 grün** (143 Files). **CR:** externes Review am PR mit `agy`. **PC:** Secret-Scan clean (nur Doku). **Eingefaltet:** #326-Reconcile (`gh`-verifiziert `mergedAt=2026-07-23T14:36:19Z`/`c4b5261`). **Kein Gate verschoben, keine Entscheidung getroffen** — das Dokument beschreibt den Ist-Stand. Risiko-Delta **null**. |

**Typ:** Doc-only Referenz (TL-12 Zustellpfad end-to-end, Gates verortet) + #326-Reconcile. Kein Deploy/Secret/Host. **DO:** `docs/architecture/TL-12-delivery-path.md`, `TODO.md`, `changes/2026-07-23_tl12-delivery-path.md`, `CHANGES.md`, dieser Eintrag.

| #328 | (base=main, gemergt) | 2026-07-24 11:05 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummer #327) → CO/CG/TS entfallen** (kein Code/Test/Design, Doku-Hygiene; Präzedenz #313/#318/#321). #327 ist seit `mergedAt=2026-07-23T14:43:19Z` gemergt (`f5677e9`), konnte aber die **eigene** Nummer beim Schreiben nicht kennen ⇒ Erst-Spalte trug noch `(offen, base=main)`, CHANGES-Überschrift keinen `#327`-Marker. 3 Annotationen, rein 1:1 in-place: COMPLIANCE-Erst-Spalte → `#327` + `(base=main, gemergt)`; CHANGES-Überschrift `, #327)`; TODO-Eintrag annotiert. **Verifikation:** `gh pr view 327` → `MERGED` mit Titel-/Timestamp-Match; danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser; Suite unverändert **2045 grün** (kein `.ts`-Diff). **CR:** externes Review am PR + Self-CR. **PC:** Secret-Scan clean (nur Doku). Kein Code/State/Deploy/Secret, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummer #327 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `TODO.md`, `changes/2026-07-24_reconcile-pr-327.md`, dieser Eintrag.

| #329 | (base=main, gemergt) | 2026-07-24 16:05 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Runbook-Ergänzung (TL-11-Runbook nennt den optionalen Daemon-Sweep `TLMCP_WAKE_SWEEP_ENABLED`) → CO/CG/TS entfallen** (kein Code/Test-Diff, **kein Flag-Flip**, keine Entscheidung; `clink`/`gemini` nicht im PATH). **Die Lücke:** #326 verdrahtete den Reconciliation-Sweep hinter `TLMCP_WAKE_SWEEP_ENABLED` (Default AUS); dokumentiert war das nur in den Architektur-Dokumenten (ADR-047 §3, Consumer-Contract §7.3), **nicht** im operativen `RUNBOOK-TL-11-wake-supervisor.md`, das der Supervisor-Betreiber liest — ein daemon-seitiges Sweep-Wake hätte er für einen Bug halten oder mit seiner eigenen Cold-Start-Pflicht verwechseln können. **Neue §5.1:** Default AUS, Auslöser `agentRegistry.on('register')` (nur die **registrierende** Instanz), Wake-Form **identisch** zum regulären Wake (Zero-Content, `reason:'inbox'`, gerichtet — nicht unterscheidbar), plus die ausdrückliche Klarstellung, dass das Flag die **Cold-Start-Sweep-Pflicht des Supervisors NICHT ersetzt**, sondern ergänzt (der Daemon sieht keinen Supervisor-Neustart ohne Re-Registrierung), und dass das Setzen ein **bewusster Owner-Schritt** ist. §7-Checkliste um denselben Hinweis ergänzt. **Alle Aussagen gegen den gemergten Code verifiziert** (`index.ts:1125` Flag-Gate; `sweep-wiring.ts` Auslöser/`reason`/Zielgenauigkeit). **TS:** kein `.ts`-Diff, Suite unverändert **2045 grün** (143 Files). **CR:** externes Review am PR mit `agy`. **PC:** Secret-Scan clean (das Flag ist kein Secret). **Unverändert gated:** der Flag-Flip in einer laufenden Instanz (owner), TL-11 Slice B (Host-Hop), TL-12/TL-14a/TL-08/TL-10 (Sign-off/CO) — dieser Slice berührt **keines** davon. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Runbook-Ergänzung (TL-11 optionaler Daemon-Sweep — kein Flag-Flip, kein Deploy). Kein Secret/Host. **DO:** `docs/RUNBOOK-TL-11-wake-supervisor.md`, `changes/2026-07-24_tl11-runbook-sweep.md`, `CHANGES.md`, dieser Eintrag.

| #330 | (base=main, gemergt) | 2026-07-24 18:16 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #328 + #329) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene; Präzedenz #313/#318/#321/#328; `clink`/`gemini` nicht im PATH). **Kontext:** der zuvor erbetene #327-Nachtrag war **bereits durch #328 erledigt** (COMPLIANCE-Zeile 1969 trägt `#327`, CHANGES-Eintrag `…, #327)`); ein erneutes Nachtragen hätte nur Duplikate erzeugt. Offen war die **nächste** Stufe: #328 (`PR-Nummern-Nachtrag für #327`, `gh`→`MERGED`, `mergedAt=2026-07-24T09:09:25Z`, `e06ce3c`) und #329 (`TL-11-Runbook optionaler Daemon-Sweep`, `gh`→`MERGED`, `mergedAt=2026-07-24T14:08:34Z`, `d61174f`) trugen ihre **eigene** Erst-Spalte noch als `(offen, base=main)`. **Was:** COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#328`/`#329` + `(base=main, gemergt)`; CHANGES-Überschriften um `, #328)`/`, #329)` ergänzt — 1:1 in-place, `gh`-verifiziert. Danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser (dieselbe Selbst-Reconcile-Lücke, fällt im nächsten Nachtrag). **TS:** kein `.ts`-Diff, Suite unverändert **2045 grün** (143 Files). **CR:** externes Review am PR (Bot-Pfad) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453 (Ball bei Christian). Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #328 + #329 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-24_reconcile-pr-328-329.md`, dieser Eintrag.

| #331 | (base=main, gemergt) | 2026-07-25 06:05 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Code+Test-Slice (TL-11 konsumentenseitiger Wake-Kern `interpretWakeFrame`, rein, **0 Aufrufer**) → CO/CG entfallen** (keine neue Design-Entscheidung; alle Werte aus dem gemergten Kontrakt #271/#277 abgeleitet, in Consumer-Contract §6.1 dokumentiert; `clink`/`gemini` nicht im PATH). **Die Lücke:** die daemon-seitige Wake-Erzeugung/-Zustellung ist end-to-end getestet, aber was der Out-of-Repo-Supervisor (Slice B) aus einem Frame macht, stand **nur als Pseudocode** (Consumer-Contract §6). Dieser Pseudocode mischt **Transport** (WS/mTLS/Reconnect — host-gebunden, out-of-repo) und **Frame-Interpretation** (rein, host-frei) — der reine Teil war ungetestet; belegbar kein Kosmetik-Punkt, weil die frühere §6-Fassung `ev.reason` statt `ev.data.reason` las (Wire-Shape-Befund #282) und diese Fehlklasse **konsumentenseitig keinen Test** hatte. **Neu `packages/daemon/src/wake-consumer-reference.ts`** (Form wie `sweep-targets.ts`: reiner Kern unter weiterhin gatetem letzten Hop, 0 Aufrufer ⇒ kein Runtime-Delta): `interpretWakeFrame(raw)` → poke ja/nein, liest den Payload **ausschließlich unter `.data`** (§4), tolerant gegen unbekannte `reason` (§4), nur `agent:wake` pokt (§3 Event-Typ-Filter), Zero-Content gewahrt (nur `instance_id`/`spiffe_uri`/`reason`), **wirft nie**; `coldStartSweepDecision()` benennt die §5-Cold-Start-Pflicht als Trigger. **TS ✅:** +20 Tests (`wake-consumer-reference.test.ts`), **mutations-verifiziert** am Wire-Shape-Guard (liest der Kern Top-Level-`reason`, wird der Test rot: `expected 'TOPLEVEL' to be 'inbox'`); `tsc --noEmit` grün, `eslint` grün, Full Suite **2065 grün** (144 Files, +20 ggü. 2045). **CR:** Self-Review (agy/`pal:codereview`-Backend nicht installiert, s. Memory; Subagent-CR in dieser Session nicht genutzt) — keine HIGH/MEDIUM: fail-safe auf nicht-String-`type`, keine `undefined`-Zuweisung unter strict optional types, Zero-Content strukturell über Feld-Auswahl; externes `agy`-Review am PR nachzuziehen. **PC:** Secret-Scan clean (reiner Interpreter, keine I/O). **De-riskt** Slice B (Supervisor-Autor bekommt den reinen Teil kopierbar+getestet), **entfernt** den Blocker NICHT — Transport + `pokeCli`-Body + Zwei-Peer-Live-Proof bleiben out-of-repo. **Nicht berührt:** ADR-046 §9, msg 1453. Kein Deploy/Secret/State, additiv. |

**Typ:** Code+Test-Slice (TL-11 konsumentenseitiger Wake-Kern — rein, 0 Aufrufer, kein Runtime-Delta). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/wake-consumer-reference.ts` (+Test), `docs/architecture/TL-11-wake-consumer-contract.md` §6.1, `TODO.md`, `CHANGES.md`, `changes/2026-07-24_tl11-wake-consumer-reference-core.md`, `COMPLIANCE-TABLE.md`.

| #332 | (base=main, gemergt) | 2026-07-25 06:21 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #330 + #331) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene; Präzedenz #313/#318/#321/#328/#330; `clink`/`gemini` nicht im PATH). **Kontext:** der ggf. vermutete #328-Nachtrag war **bereits durch #330 erledigt** (Zeile 1973 trägt `#328`, CHANGES `…, #328)`); offen waren die **beiden nächsten** Zeilen: #330 (`PR-Nummern-Nachtrag für #328 + #329`, `gh`→`MERGED`, `mergedAt=2026-07-24T16:33:17Z`, `f58b8d1`) und #331 (`feat(tl11) interpretWakeFrame`, `gh`→`MERGED`, `mergedAt=2026-07-25T04:17:37Z`, `340b7c2`), beide trugen ihre **eigene** Erst-Spalte noch als `(offen, base=main)`. **Was:** COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#330`/`#331` + `(base=main, gemergt)`; CHANGES-Überschriften um `, #330)`/`, #331)` ergänzt — 1:1 in-place, `gh`-verifiziert. Danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2065 grün** (144 Files). **CR:** externes Review am PR (Bot-Pfad) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453 (Ball bei Christian), TL-12-Gates. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #330 + #331 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-25_reconcile-pr-330-331.md`, dieser Eintrag.

| #333 | (base=main, gemergt) | 2026-07-25 06:28 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Test-only-Slice (TL-11 Emit→Decision-Brücke) → CO/CG entfallen** (keine Design-Entscheidung; Test gegen bereits gemergten Code #271/#277/#331; `clink`/`gemini` nicht im PATH). **Die Lücke:** drei TL-11-Wake-Test-Ebenen existierten (Emitter-Funktionen `wake-contract.test.ts` · Emitter über echten `/ws`-Socket `tl11-wake-wire.conformance.test.ts` · Kern `interpretWakeFrame` gegen **hand-gebaute** Frames `wake-consumer-reference.test.ts`), aber **keine** verband die beiden Enden: entscheidet der Kern (#331) korrekt über den Frame, den der Daemon **tatsächlich emittiert und serialisiert**? Ein Drift genau dazwischen (Payload flach statt unter `.data`, `MeshEventBus`-Hülle geändert, Inhaltsfeld leckt) wäre in allen drei Ebenen grün geblieben, während der Out-of-Repo-Supervisor (Slice B) falsch entschiede. **Neu `packages/daemon/src/tl11-wake-emit-to-decision.test.ts`:** fährt **socket-frei** die volle Kette `inbox:new` → echter `registerWakeEmitter` → echter `MeshEventBus` (`{type,timestamp,data}`) → **dieselbe Serialisierung wie der Draht** (`websocket.ts:266` `JSON.stringify(event)` aus dem `onAny`-Kanal `:265`) → `interpretWakeFrame`; Uhr+Live-Liste injiziert. Deckt: adressiert+live+SPIFFE → 1 Frame poket korrekt · `.data`-Hülle · **Zero-Content** durch die ganze Kette (weder `message_id` noch `body` reisen mit, Draht **und** Decision) · coalesced (2→1) · fail-closed ohne SPIFFE / Ziel nicht live → 0 Frames. **Abgrenzung:** der Transport (Socket/mTLS/Loopback-Gate/Routing) ist NICHT Gegenstand (deckt der Conformance-Test) — hier allein die Konsistenz **Emit-Bytes ↔ Konsumenten-Entscheidung**. **TS ✅:** +6 Tests, **mutations-verifiziert** (Kern liest Top-Level statt `.data` ⇒ Weld-Test rot: `instance_id`/`spiffe_uri` kommen `undefined`); `tsc`+`eslint` grün, Full Suite **2071 grün** (145 Files, +6 ggü. 2065). **CR:** Self-Review, keine Tautologie (Mutation beißt, Test fährt echte Produktionspfade); externes `agy`-Review am PR. **PC:** Secret-Scan clean (Test-Konstanten; `body:'top-secret-payload'` ist ein Negativ-Marker). **De-riskt** Slice B weiter, **entfernt** den Blocker NICHT (Transport + `pokeCli`-Body bleiben out-of-repo). **Nicht berührt:** ADR-046 §9, msg 1453, TL-12-Gates. Test-only, kein Deploy/Secret/State, keine Produktionsdatei berührt. |

**Typ:** Test-only-Slice (TL-11 Emit→Decision-Brücke — keine Produktionsdatei berührt). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/tl11-wake-emit-to-decision.test.ts`, `docs/architecture/TL-11-wake-consumer-contract.md` §6.1, `TODO.md`, `CHANGES.md`, `changes/2026-07-25_tl11-wake-emit-to-decision-bridge.md`, `COMPLIANCE-TABLE.md`.

| #334 | (base=main, gemergt) | 2026-07-25 06:35 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only (TL-11 §6-Referenz-Konsument an die getesteten Primitive angebunden) → CO/CG/TS entfallen** (kein Code/Test-Diff, keine Entscheidung, kein Gate verschoben; `clink`/`gemini` nicht im PATH). **Die Lücke:** der §6-Referenz-Konsument (`TL-11-wake-consumer-contract.md`) zeigte die Frame-Interpretation als hand-ausgeschriebenes `JSON.parse` + `ev.type==='agent:wake'` + `ev.data?.reason` — genau die Stelle mit der früheren `ev.reason`-statt-`ev.data.reason`-Fehlklasse (#282). Seit #331 gibt es dafür einen getesteten, mutations-verifizierten Kern (`wake-consumer-reference.ts`, §6.1), aber die **kopierbare** Referenz nutzte ihn nicht — ein Supervisor-Autor hätte die Interpretation neu (und ggf. neu falsch) geschrieben. **Was:** §6-Pseudocode auf die Primitive umgestellt (`interpretWakeFrame`/`coldStartSweepDecision`), Import-Kommentar (Supervisor vendored den getesteten Kern), Abschlusszeile benennt jetzt **Transport UND `pokeCli`** als die zwei Slice-B-Teile. **Keine Verhaltens-/Vertragsänderung** — dieselben §3/§4/§5-Garantien, nur an getesteten Code gebunden. **TS:** kein `.ts`-Diff, Suite unverändert **2071 grün** (145 Files). **CR:** Bot-Pfad-Review + Self-CR (Primitive/Signatur gegen `wake-consumer-reference.ts` geprüft). **PC:** Secret-Scan clean (nur Doku). **De-riskt** den letzten Hop weiter (kopierbare Referenz correct-by-construction), **entfernt** den Blocker NICHT. **Nicht berührt:** ADR-046 §9, msg 1453, TL-12-Gates. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only (TL-11 §6-Referenz-Konsument an getestete Primitive gebunden — kein Code/Test). Kein Deploy/Secret/Host. **DO:** `docs/architecture/TL-11-wake-consumer-contract.md` §6, `TODO.md`, `CHANGES.md`, `changes/2026-07-25_tl11-ref-loop-primitives.md`, `COMPLIANCE-TABLE.md`.

| #335 | (base=main, gemergt) | 2026-07-25 07:22 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Test-only-Slice (TL-12 Slice A / S5 Read-Surface am HTTP-Rand gehärtet) → CO/CG entfallen** (keine Design-Entscheidung; Test gegen bereits gemergten Code ADR-038; `clink`/`gemini` nicht im PATH). **Die Lücke:** `verifyStoredOrder` (reine Re-Verify-Funktion) ist in `agent-inbox.test.ts` gut bewacht (inkl. Byte-Flip), aber der **Read-Surface-Endpunkt** `GET /api/inbox` (S5) war für Aufträge nur mittelbar getestet — nicht das `order`-Surfacing (`is_order`/`verify_verdict`) und **nicht** die Zusage `inbox-api.ts:382` („ein auf der Platte manipulierter Auftrag zeigt hier INVALID … eine bösartige Zeile legt die Liste nicht lahm"). **Neu `packages/daemon/src/tl12-order-read-surface.test.ts`:** fährt `GET /api/inbox` via `fastify.inject()` gegen eine echte `AgentInbox` und beweist S5 end-to-end: (a) gültiger Auftrag → `is_order=true`+`verify_verdict=VALID`+Provenienz · (b) **echte On-Disk-Manipulation** (zweite `better-sqlite3`-Verbindung kippt ein `signed_bytes`-Byte, wie ein Angreifer mit Plattenzugriff) → Live-Re-Verify beim Lesen dreht `VALID→INVALID`, Endpunkt bleibt `200` · (c) fail-closed-Loop: die manipulierte Zeile verschluckt die Plain-Nachricht nicht (`count` vollständig). **Abgrenzung: S6 (Abarbeitung) NICHT Gegenstand** — owner-gated, nicht gebaut; nur das Lese-Verdikt (S5). **TS ✅:** +3 Tests, **mutations-verifiziert** (Handler gäbe die gespeicherte `verify_verdict`-Spalte statt live `verifyStoredOrder` ⇒ On-Disk-Tamper + fail-closed-Loop rot: `expected 'VALID' to be 'INVALID'`); `tsc`+`eslint` grün, Full Suite **2074 grün** (146 Files, +3 ggü. 2071). **CR:** Self-Review, keine Tautologie (Mutation beißt zweifach, echter Endpunkt + echte DB-Manipulation); externes `agy`-Review am PR. **PC:** Secret-Scan clean (In-Memory-`generateKeyPairSync`, keine echten Keys). **Nicht berührt:** TL-12 S6/Slice B (owner/CO), ADR-046 §9, msg 1453. Test-only, kein Deploy/Secret/State, keine Produktionsdatei berührt. |

**Typ:** Test-only-Slice (TL-12 S5 Read-Surface-Härtung — keine Produktionsdatei berührt). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/tl12-order-read-surface.test.ts`, `docs/architecture/TL-12-delivery-path.md` §S5, `TODO.md`, `CHANGES.md`, `changes/2026-07-25_tl12-order-read-surface.md`, `COMPLIANCE-TABLE.md`.

| #336 | (base=main, gemergt) | 2026-07-25 08:16 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #332 + #333 + #334 + #335) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene; Präzedenz #313/#318/#321/#328/#330/#332; `clink`/`gemini` nicht im PATH). **Kontext:** seit #332 (zog #330/#331) sind vier PRs gemergt, deren **eigene** Erst-Spalte noch `(offen, base=main)` trug: #332 (docs(reconcile), `gh`→`MERGED`, `24de23c`), #333 (test(tl11) Emit→Decision-Brücke, `b93452b`), #334 (docs(tl11) §6-Referenz an Primitive, `069b1a9`), #335 (test(tl12) S5 Read-Surface, `c9f17b6`). **Was:** COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#332`/`#333`/`#334`/`#335` + `(base=main, gemergt)`; CHANGES-Überschriften um `, #NNN)` ergänzt — 1:1 in-place, `gh`-verifiziert MERGED. Danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2074 grün** (146 Files). **CR:** externes Review am PR (Bot-Pfad) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, TL-12 S6/Slice B/C, ADR-046-Producer, TL-11 Slice B. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #332 + #333 + #334 + #335 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-25_reconcile-pr-332-335.md`, dieser Eintrag.

| #337 | (base=main, gemergt) | 2026-07-25 12:09 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Bug-Fix-Slice (TL-10 Slice A / `freigabe-matrix.ts`: zwei #300/#319-CR-Altlasten) → CO/CG entfallen** (reiner Bug-Fix, keine Design-Entscheidung; `clink`/`gemini` nicht im PATH). **Grounding:** `TL-10-freigabe-matrix-scoping.md` §7.2 (Punkt 6) + `TODO.md` führen zwei aus dem #319-CR mitgenommene, vorbestehende (#300) Defekte, dort als „im D1-Loader zu erledigen" geparkt — beide sind aber reine Slice-A-Korrektheit und **ungegatet** fixbar (`freigabe-matrix.ts` hat **0 Runtime-Aufrufer** → nur `approval-router.ts`, selbst 0 Aufrufer ⇒ **kein Verhaltens-Delta**, kein D1/D3-Gate-Vorgriff). **Was:** (1) whitespace-only Kanalname parste (`channel.length===0` ließ `'   '` durch → nicht-zustellbare channelId, Konfig-Footgun) ⇒ Parser wirft jetzt bei `channel.trim().length===0`, plus `isRoutable`-Defense-in-depth; (2) `resolveEntry` gab `decider` **per Referenz** heraus (Aliasing: ein mutierender Konsument hätte die geladene Policy für alle Folge-Auflösungen verändert; `readonly` ist nur Compile-Zeit) ⇒ neue reine `cloneDecider`, `resolveEntry` gibt eine **frische Kopie**. Keine Vertrags-/Signaturänderung; für alle bisher gültigen Eingaben identisches Verhalten. **TS ✅:** +6 Regressionstests (jeder Defekt bewacht: 4× whitespace-`channel`→reject, decider-Kopie/Aliasing, `isRoutable`-whitespace), **mutations-verifiziert** (`trim()`→`length` ⇒ 4 Reject-Tests rot; `cloneDecider`→Referenz ⇒ Aliasing-Test rot); `tsc`+`eslint` grün, Full Suite **2080 grün** (146 Files, +6 ggü. 2074). **CR:** Self-Review, 0 Runtime-Aufrufer (kein Regress-Risiko), Fixes minimal+typkorrekt; externes `agy`-Review am PR. **PC:** Secret-Scan clean (reine Logik). **Nicht berührt:** TL-10 Slice B (D1-Loader/D3-Sign-off/Env-Flag), ADR-046 §9, msg 1453, TL-12 S6. Kein Deploy/Secret/State. |

**Typ:** Bug-Fix-Slice (TL-10 Slice A Parser-Härtung — 0 Runtime-Aufrufer, kein Verhaltens-Delta). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/freigabe-matrix.ts` (+Test), `docs/architecture/TL-10-freigabe-matrix-scoping.md` §7.2, `TODO.md`, `CHANGES.md`, `changes/2026-07-25_tl10-sliceA-parser-hardening.md`, `COMPLIANCE-TABLE.md`.

| #338 | (base=main, gemergt) | 2026-07-25 13:05 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Code+Test-Slice (TL-10 D1-Prep: reiner TOML-Text→Matrix-Loader `parseFreigabeMatrixToml`, 0 Aufrufer) → CO/CG entfallen** (keine Design-Entscheidung — die D1-Quelle `config/freigabe-matrix.toml` ist bereits per §5-CO 2026-07-20 entschieden; dies ist der **mechanische Format-Schritt**; `clink`/`gemini` nicht im PATH). **Gate-neutral:** nimmt keinen D1/D3-Entscheid vorweg, Form wie die gemergten Slice-B-Prep-Primitive #317/#319. **Neu `packages/daemon/src/freigabe-matrix-loader.ts`:** `parseFreigabeMatrixToml(tomlText, knownServers)` parst **TOML-Text** (liest **keine** Datei) via `@iarna/toml`, wrappt einen TOML-Syntaxfehler **fail-closed** in `FreigabeMatrixError` (Aufrufer fängt nur EINEN Typ) und delegiert die volle §2.2-Validierung an `parseFreigabeMatrix`; leer/tabellenlos ⇒ leere Matrix (D5). **Bewusst NICHT hier** (weiterhin gegatet): fs-Lesen der Datei, die kuratierte Policy-Datei (D1-Inhalt, owner — NICHT mitgeliefert), Ingress-Verdrahtung + Env-Flag, D3-Sign-off. **TS ✅:** +7 Tests (gültiges TOML→Matrix, leer→D5, malformed→FreigabeMatrixError, Delegation §2.2 inkl. whitespace-Kanal-Erbe #337 + non-kanonischer Server + decider-Grammatik, Nicht-String→Error); `tsc`+`eslint` grün, Full Suite **2087 grün** (147 Files, +7 ggü. 2080). **CR:** Self-Review, 0 Aufrufer (kein Regress-Risiko), delegiert die Sicherheits-Validierung unverändert; externes `agy`-Review am PR. **PC:** Secret-Scan clean (reine Logik, **keine** Policy-Datei mitgeliefert). **Nicht berührt:** TL-10 Slice B (kuratierte Datei/Verdrahtung/Env-Flag/D3), ADR-046 §9, msg 1453, TL-12 S6. Kein Deploy/Secret/State. |

**Typ:** Code+Test-Slice (TL-10 D1-Prep: reiner TOML-Loader — 0 Aufrufer, kein Runtime-Delta). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/freigabe-matrix-loader.ts` (+Test), `docs/architecture/TL-10-freigabe-matrix-scoping.md` §7.2, `TODO.md`, `CHANGES.md`, `changes/2026-07-25_tl10-d1-toml-loader.md`, `COMPLIANCE-TABLE.md`.

| #339 | (base=main, gemergt) | 2026-07-25 14:03 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #336 + #337 + #338) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene; Präzedenz #313/#318/#321/#328/#330/#332/#336; `clink`/`gemini` nicht im PATH). **Kontext:** seit #336 (zog #332–#335) sind drei PRs gemergt, deren **eigene** Erst-Spalte noch `(offen, base=main)` trug: #336 (docs(reconcile), `gh`→`MERGED`, `668b700`), #337 (fix(tl10) Slice-A-Parser-Härtung, `16de282`), #338 (feat(tl10) D1-TOML-Loader, `99e4d3d`). **Was:** COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#336`/`#337`/`#338` + `(base=main, gemergt)`; CHANGES-Überschriften um `, #NNN)` ergänzt — 1:1 in-place, `gh`-verifiziert MERGED. Danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2087 grün** (147 Files). **CR:** externes Review am PR (Bot-Pfad) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, TL-10 Slice B, TL-12 S6/Slice B/C, TL-11 Slice B. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #336 + #337 + #338 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-25_reconcile-pr-336-338.md`, dieser Eintrag.

| #340 | (base=main, gemergt) | 2026-07-26 07:16 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (#339-Nachtrag + TODO-PR-Nummern-Aufholung #331→#339) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene; `clink`/`gemini` nicht im PATH). **Zwei Lücken nach der Serie #331–#339:** (1) **#339** (Reconcile für #336–#338) kannte seine **eigene** Merge-Nummer nicht ⇒ Erst-Spalte `(offen, base=main)` → `#339` + `(base=main, gemergt)`, CHANGES-Überschrift `…, #339)`; (2) **`TODO.md` hing >5 PRs zurück** — die Slices #331–#338 standen mit Datum, aber **ohne** Merge-PR-Nummer, während `COMPLIANCE-TABLE.md`/`CHANGES.md` sie schon trugen (Doku-Kanon driftete gegeneinander). **TODO-PR-Nummern nachgezogen** (Datum → Datum + `#NNN`): #331 (Wake-Kern) · #333 (Emit→Decision-Brücke) · #334 (§6-Referenz) · #335 (S5 Read-Surface) · #337 (Slice-A-Altlasten) · #338 (D1-Loader) — die reinen Reconcile-PRs #332/#336/#339 haben keinen TODO-Eintrag. Danach spiegeln COMPLIANCE/CHANGES/TODO dieselben Nummern für #331–#338, und **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2087 grün** (147 Files). **CR:** externes Review am PR (Bot-Pfad) + Self-CR (`gh pr view`-abgeglichen). **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, TL-10 Slice B, TL-12 S6/Slice B/C, TL-11 Slice B. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (#339-Nachtrag + TODO-Aufholung #331→#339 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `TODO.md`, `changes/2026-07-26_reconcile-pr-339-todo-catchup.md`, dieser Eintrag.

| #341 | (base=main, gemergt) | 2026-07-27 06:47 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Grounding-Doc + Charakterisierungs-Test (TL-14a Auflage C / Revocation) → CO/CG entfallen** (Grounding leitet aus gemergtem Code/Consensus ab, entscheidet nichts; non-gated Vorbedingungs-Lane wie A/B; `clink`/`gemini` nicht im PATH). **Die Lücke:** der TL-14a-Consensus (`TL-14a-consensus-result-D1-D6.md` §C) führt **Auflage C — „keine Revocation-Infra"** als **blockierend**; **A** und **B** haben ein Grounding + gemergte Slices (#297/#298/#299/#311), **C war die einzige der drei blockierenden Auflagen ohne Grounding**. **Neu `TL-14a-blocker-C-grounding.md`** (code-verifiziert): `crl.ts` (`CertificateRevocationList`, `revoke`/`isRevoked`, datei-persistiert) **existiert bereits** als genau die vom Consensus vorgeschlagene gepinnte Fingerprint-Denylist, ist aber **0-Aufrufer-Dead-Code** (`grep "from './crl"` ohne Tests = 0) obwohl der Kopf „geprüft beim Heartbeat/Agent-Card" behauptet (nicht verdrahtet); Hook-Kandidat vorhanden (`agent-card.ts:311,353` Peer-Fingerprint). **Aufgedeckter Widerspruch:** Consensus stuft A–C blockierend ein, **ADR-045 §74 führt nur A/B**. **Neu `crl.test.ts`** charakterisiert das bislang **ungetestete** Ist-Verhalten (+6: revoke/isRevoked/unrevoke/size/list, Persistenz-Roundtrip, fail-safe Load fehlend/korrupt, atomarer Write) — **keine Verdrahtung** (A-Pattern wie `tls-chain-characterization.test.ts`). **Offen (CO/Owner, NICHT entschieden):** Denylist vs CRL/OCSP · `crl.ts` verdrahten vs Neubau · C-Blocking-Status vs ADR-045 · Distribution/Pinning. **TS ✅:** +6 Tests, `tsc`+`eslint` grün, Full Suite **2093 grün** (148 Files, +6 ggü. 2087). **CR:** Self-Review, Test charakterisiert nur (keine `crl.ts`-Verhaltensänderung), Note verdrahtet nichts; externes `agy`-Review am PR. **PC:** Secret-Scan clean (synthetische Test-Fingerprints). **Nicht berührt:** TL-14a Christian-Sign-off/ADR-045/Runbook/TL-14b, ADR-046 §9, msg 1453, TL-08/09/10-Gates. Kein Deploy/Secret/State. |

**Typ:** Grounding-Doc + Charakterisierungs-Test (TL-14a Auflage C — keine Verdrahtung, keine Entscheidung). Kein Deploy/Secret/Host. **DO:** `docs/architecture/TL-14a-blocker-C-grounding.md`, `packages/daemon/src/crl.test.ts`, `TODO.md`, `CHANGES.md`, `changes/2026-07-27_tl14a-blocker-C-grounding.md`, `COMPLIANCE-TABLE.md`.

| #342 | (base=main, gemergt) | 2026-07-27 07:19 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Test-only-Slice (TL-14a Vorbedingung A — pathLen auf der Transport-Ebene) → CO/CG entfallen** (keine Design-Entscheidung; Test gegen gemergten Code/ADR-045-Vorbedingung; non-gated Lane, D3-unabhängig; `clink`/`gemini` nicht im PATH). **Die Lücke:** ADR-045 §77 verlangt einen pathLen-Test **auf der Transport-Ebene**; App-Ebene war gedeckt (`verifyPeerCertChain`, #298/#299/#311) + flache Lücke charakterisiert (#295), aber der **Transport-Test fehlte** (`chain-verify.test.ts` = reine Funktion, 0 Handshakes; `mtls-issuer-fingerprint.test.ts` = Issuer-Fingerprint, keine mehrstufige Kette). **Neu `tls-transport-pathlen.conformance.test.ts`** (echter Node-TLS-Handshake, `requestCert`+`rejectUnauthorized`): **BEFUND — Node-TLS erzwingt `pathLenConstraint` NICHT** — `Root(pathLen 0)→Intermediate→Leaf` **und** `Root(p2)→Inter(p0)→Sub-CA→Leaf` werden am Transport **akzeptiert** (Positiv-Kontrolle pathLen 1 = kein Harness-Fehler); **Kontrast im selben Test:** die App-Ebene `verifyPeerCertChain` **lehnt** denselben Verstoß **ab**. ⇒ Zwei-Stufen-Trust MUSS über die App-Ebene laufen (D2/D4 nicht am Transport); macht ADR-045s „D2 kosmetisch/ungetestet" regressionsfest ⇒ **Vorbedingung A code-seitig komplett** (B ist bereits fertig, #297). **TS ✅:** +4 Tests, **Befund per Throwaway-Probe doppelt verifiziert** (root- **und** intermediate-pathLen beide akzeptiert); `tsc`+`eslint` grün, Full Suite **2097 grün** (149 Files, +4 ggü. 2093). **CR:** Self-Review; **claude/codex/agy** am PR (NIE MiniMax/pal:chat). **PC:** Secret-Scan clean (synthetische Test-Zertifikate). **Nicht berührt:** D3-Sign-off/ADR-045-Status/Runbook/TL-14b, C-Entscheidung, ADR-046 §9, msg 1453, TL-08/09/10-Gates. Test-only, kein Deploy/Secret/State, keine Produktionsdatei berührt. |

**Typ:** Test-only-Slice (TL-14a Vorbedingung A — pathLen am Transport, echter mTLS-Handshake; keine Produktionsdatei). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/tls-transport-pathlen.conformance.test.ts`, `docs/architecture/ADR-045-ca-two-stage-hierarchy.md` §77, `TODO.md`, `CHANGES.md`, `changes/2026-07-27_tl14a-A-transport-pathlen.md`, `COMPLIANCE-TABLE.md`.

| #343 | (base=main, gemergt) | 2026-07-27 07:50 | n/a | n/a | n/a | ⏳ | ✅ | ✅ | **Reine TODO-Notiz: TL-12 B1-Reihenfolge-Pflicht (Keyid-Backfill VOR der `UNIQUE`-Bedingung) → CO/CG/TS entfallen** (kein Design-Diff, kein Code, kein Test; Befund-Verankerung aus dem #323-Review, Präzedenz doc-only #318/#336/#339). **Der Befund:** B1 will den kanonischen DER-SPKI-Keyid auf `UNIQUE(signer_keyid, order_nonce)` legen, aber `signer_keyid` wird **heute schon** mit dem format-malleablen **PEM-Hash** aus `orderKeyId` befüllt (`index.ts:876`), persistiert (`agent-inbox.ts:315/336`) und über `idx_messages_order` indiziert (`agent-inbox.ts:188/219`). **Reihenfolge-Bedingung:** Altzeilen auf den kanonischen Keyid **backfillen, BEVOR** B1 die Spaltensemantik umstellt und die `UNIQUE`-Bedingung setzt — sonst kollidiert ein wiedereingespielter Alt-Auftrag (Altzeile = PEM-Hash) nicht mit seiner neuen kanonischen Zeile ⇒ **Übergangsloch im Replay-Schutz**, ausgerechnet im einführenden Slice. **Backfill ohne Neu-Sammeln möglich:** `signer_pubkey` liegt unveränderlich in derselben Zeile (`agent-inbox.ts:73`, trust-on-first-verify) ⇒ kanonischer Keyid je Altzeile aus `signer_pubkey` nachrechenbar (`canonicalOrderKeyId`; `null` ⇒ Zeile als nicht-backfillbar markieren, **kein** PEM-Fallback). **Alle Fakten gegen aktuellen `origin/main` `git grep`-verifiziert.** **CR ⏳:** externes `agy`-Review am PR (agent-codex OAuth abgelaufen). **PC:** Secret-Scan clean (nur Doku, keine Werte). **Kein Schema-Eingriff, keine Migration, kein `.ts`-Diff, keine der vier §9-Entscheidungen berührt** — Slice B bleibt vollständig gated. Suite unverändert (kein Testlauf nötig). |

**Typ:** Reine TODO-Notiz (TL-12 B1-Reihenfolge — kein Code/Test/Design, kein Schema/Migration). Kein Deploy/Secret/Host. **DO:** `TODO.md`, `CHANGES.md`, `changes/2026-07-27_tl12-b1-backfill-before-unique.md`, `COMPLIANCE-TABLE.md`.

| #344 | (base=main, gemergt) | 2026-07-27 07:51 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **Refactor+Test-Slice (TL-14a Vorbedingung B — Monitor-Verdrahtung regressionsfest) → CO/CG entfallen** (behavior-preserving Refactor + Test, keine Design-Entscheidung; gate-frei/D3-unabhängig; `clink`/`gemini` nicht im PATH). **KEIN Doppel zu #297:** die B-Funktionsteile (`getCaCertDaysLeft` + `subject`-Label + `ca-cert-expiry.test.ts`) sind seit #297 gemergt — dies schließt die **ungetestete Verdrahtung**. **Die Lücke:** die Auswahl „welcher Monitor liest welche Quelle" lag als zwei fast identische `startCertExpiryMonitor(...)`-Blöcke **inline in `index.ts`** und war **ungetestet** — ein „beide lesen `node.crt.pem`" oder ein weggefallener CA-Monitor wäre unsichtbar. **Was:** neu reine `cert-monitor-wiring.ts` `buildCertExpiryMonitorSpecs(dataDir, thresholds)` (Node via `getCertDaysLeft`/`subject:'Node'` + CA/Intermediate via `getCaCertDaysLeft`/`subject:'CA'`, kein I/O beim Bauen); `index.ts` mapt darüber (`startCertExpiryMonitor({...spec,log,audit,eventBus})`, Shutdown `forEach clearInterval`) — **verhaltensbewahrend** (identische Monitore/Quellen/Subjects/Schwellen/Intervall). **TS ✅:** +4 Tests (`cert-monitor-wiring.test.ts`: 2 Specs subjects `[Node,CA]`, Schwellen durchgereicht, **getrennte Quellen bewiesen** — nur node.crt.pem ⇒ Node-Quelle liefert/CA null & umgekehrt, kein I/O beim Bauen); `tsc` grün, neue Dateien `eslint`-grün (der 1 vorbestehende `index.ts:293`-Error ist nicht aus diesem Slice), Full Suite **2101 grün** (150 Files, +4 ggü. 2097) — der index.ts-Refactor brach keine Test. **CR:** Self-Review; **claude/agy** am PR (codex OAuth-Ausfall), NIE MiniMax/pal:chat. **PC:** Secret-Scan clean (In-Memory-Testzertifikate). ⇒ **Vorbedingung B code-seitig komplett + verdrahtungsfest.** **Nicht berührt:** D3-Sign-off/ADR-045-Status/Runbook/TL-14b, C-Entscheidung, ADR-046 §9, msg 1453, TL-08/09/10-Gates. |

**Typ:** Refactor+Test-Slice (TL-14a Vorbedingung B — Monitor-Verdrahtung regressionsfest; verhaltensbewahrend). Kein Deploy/Secret/Host. **DO:** `packages/daemon/src/cert-monitor-wiring.ts` (+Test), `packages/daemon/src/index.ts` (Refactor), `TODO.md`, `CHANGES.md`, `changes/2026-07-27_tl14a-B-monitor-wiring.md`, `COMPLIANCE-TABLE.md`.

| #345 | (base=main, gemergt) | 2026-07-27 08:16 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummer #340) → CO/CG/TS entfallen** (kein Code/Test/Design, reine Doku-Hygiene laut Reconcile-Bericht 2026-07-27 03:30; Präzedenz #328/#330/#332/#336/#339; `clink`/`gemini` nicht im PATH). **Die Lücke:** #340 (`docs(reconcile): #339-Nachtrag + TODO-PR-Nummern-Aufholung`, `gh`→`MERGED`, `mergedAt=2026-07-26T05:20:35Z`, `53dfa2f`) kannte seine **eigene** Merge-Nummer beim Schreiben nicht ⇒ Erst-Spalte blieb `(offen, base=main)`, CHANGES-Überschrift ohne `#340`-Marker. **Was:** COMPLIANCE-Erst-Spalte `(offen, base=main)` → `#340` + `(base=main, gemergt)`; CHANGES-Überschrift `(2026-07-26 07:16)` → `(…, #340)` — 1:1 in-place, `gh`-verifiziert, **keine neue Erfindung**. (Die jüngeren #343/#344-Erst-Spalten sind ebenfalls offen und fallen in einen späteren Batch — nicht Gegenstand dieses vom 03:30-Bericht scoped #340-Slices.) **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** externes Review am PR (Bot-Pfad) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, alle offenen Gates. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummer #340 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-27_reconcile-pr-340.md`, dieser Eintrag.

| #346 | (base=main, gemergt) | 2026-07-27 08:42 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #341 … #345) → CO/CG/TS entfallen** (kein Laufzeitcode/Test/Design, reine Doku-Hygiene; Präzedenz #328/#330/#332/#336/#339/#340; `clink`/`gemini` nicht im PATH). **Kontext:** der Dispatch nannte #343/#344/#345 — beim Zählen fielen zusätzlich **#341 + #342** als noch offen auf; um die trailing Drift **vollständig** zu schließen, deckt dieser PR **#341 … #345** ab: #341 (docs+test(tl14a) Auflage-C-Grounding, `gh`→`MERGED`, `dd2c4e9`), #342 (test(tl14a) A-Transport-pathLen, `873c2d2`), #343 (docs(tl12) B1-Reihenfolge, `ba9a143`), #344 (refactor+test(tl14a) B-Monitor-Verdrahtung, `f52af5d`), #345 (docs(reconcile) #340-Nachtrag, `14b90ad`). **Was:** COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#NNN` + `(base=main, gemergt)`; CHANGES-Überschriften um `, #NNN)` ergänzt — 1:1 in-place, `gh`-verifiziert MERGED. Danach **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** Review-of-Record über **agy** (Gemini 3.1 Pro — zulässig; nie MiniMax/pal:chat) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, alle offenen Gates. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #341 … #345 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `changes/2026-07-27_reconcile-pr-341-345.md`, dieser Eintrag.

| #347 | (base=main, gemergt) | 2026-07-27 09:45 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only (TL-14a: ADR-045-Klassifikations-Hinweis zu Auflage C) → CO/CG/TS entfallen** (kein Code/Test-Diff, **kein Beschluss**; WOCHENPLAN-KW31 Z. 115/116 „TL-14 = wichtigster Folgepunkt … sauberer nächster Befund statt diffuser Restwärme", Z. 111 „keine neue Phase herbeiphantasieren"; `clink`/`gemini` nicht im PATH). **Der Befund:** die #341-C-Grounding-Note deckte auf, dass der Consensus (`TL-14a-consensus-result-D1-D6.md` §C) **A–C alle drei blockierend** einstuft, **ADR-045** aber nur **A/B** unter §74 führt und **C** in §100 stumm als „out of scope" behandelt — der Widerspruch stand bislang **nur** extern, die ADR selbst schwieg. **Was:** neuer **Klassifikations-Hinweis in ADR-045 §100** — benennt die (bewusste, aber **nicht owner-ratifizierte**) Herabstufung, verweist auf die C-Grounding-§2 (Ist-Zustand `crl.ts` = gebaute 0-Aufrufer-Denylist), und **entscheidet nichts** (ob C TL-14b-Blocker/Fast-Follow ist = offener CO/Owner-Punkt). Macht die ADR ehrlich statt still. **Warum gate-frei:** die A/B/C-Vorbedingungs-Lane ist code-seitig abgetragen (#341/#342/#344 + #297/#298/#299/#311); der einzige verbleibende gate-freie Schritt ist Doku-Ehrlichkeit, **ohne** die C-/D3-Owner-Entscheidung vorwegzunehmen. **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** Review-of-Record über **claude/agy** (nie Codex/MiniMax/pal:chat) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Unverändert gated:** C-Entscheidung selbst, D3-Sign-off/ADR-045-Status/Runbook/TL-14b. **Nicht berührt:** ADR-046 §9, msg 1453, TL-08/09/10-Gates. Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only (TL-14a ADR-045-Klassifikations-Hinweis zu Auflage C — kein Beschluss). Kein Deploy/Secret/Host. **DO:** `docs/architecture/ADR-045-ca-two-stage-hierarchy.md` §100, `TODO.md`, `CHANGES.md`, `changes/2026-07-27_tl14a-adr045-C-classification-note.md`, `COMPLIANCE-TABLE.md`.

| #348 | (base=main, gemergt) | 2026-07-28 06:15 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Post-Merge-Reconcile (PR-Nummern #346 + #347 + TODO-Aufholung #338→#347) → CO/CG/TS entfallen** (kein Laufzeitcode/Test/Design, reine Doku-Hygiene; Präzedenz #328/#330/#332/#336/#339/#340/#345; `clink`/`gemini` nicht im PATH). **Die Lücke:** eine PR kennt ihre eigene Merge-Nummer beim Schreiben nicht ⇒ die beiden jüngsten Merges blieben offen — **#346** (docs(reconcile) #341–#345, `gh`→`MERGED`, `923142f`, `2026-07-27T07:45:05Z`) und **#347** (docs(tl14a) ADR-045-Klassifikations-Hinweis, `0345546`, `2026-07-27T08:45:12Z`); zusätzlich hing `TODO.md` bei #338, während COMPLIANCE/CHANGES schon #341–#345 trugen. **Was:** (1) COMPLIANCE-Erst-Spalte je `(offen, base=main)` → `#NNN` + `(base=main, gemergt)` (08:42→#346, 09:45→#347); (2) CHANGES-Überschriften um `, #NNN)` ergänzt; (3) `TODO.md` #338→#347 aufgeholt — substantielle Task-Zeilen mit Merge-Nummer versehen: #341 (C-Grounding) · #342 (A-pathLen) · #343 (tl12 B1) · #344 (B-Monitor-Verdrahtung) · #347 (ADR-045-Hinweis); die reinen Reconcile-PRs #340/#345/#346 haben keinen TODO-Task-Eintrag. Danach spiegeln COMPLIANCE/CHANGES/TODO dieselben Nummern bis #347, und **0** stale `(offen, base=main)`-Erst-Spalten mit PR-Bezug außer dieser. **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** Review-of-Record über **agy** (Gemini 3.1 Pro — zulässig; nie MiniMax/pal:chat) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, alle offenen Gates (TL-08-Gate-Flip, TL-09-Aktivierung, TL-14a-C/D3/Runbook/TL-14b). Kein Deploy/Secret/State, Risiko-Delta **null**. |
| #349 | (base=main, gemergt) | 2026-07-28 18:04 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only (TL-14a: Gate-Status-Snapshot + kein-non-gated-Slice-Befund) → CO/CG/TS entfallen** (kein Code/Test-Diff, **kein Beschluss**, kein Gate verschoben; Watchdog-scoped KW31 „Gate-Stand in Repo-Wahrheit + non-gated-Slice-Prüfung, sonst sauber parken"; kein Christian-Escalate — Telegram-Entwurf verworfen; Präzedenz doc-only #345/#346/#347; `clink`/`gemini` nicht im PATH). **Der Befund:** der TL-14a-Stand lag verstreut über TODO §398–499 (`[~]`), ADR-045 und drei Grounding-Notes — ein prüfbarer Ein-Blick-Snapshot fehlte. **Was:** neue `docs/architecture/TL-14a-gate-status.md` konsolidiert es: die agent-ausführbare Lane ist **erschöpft** — Vorbedingung **A** (`verifyPeerCertChain` + `tls-transport-pathlen.conformance.test.ts`, #298/#311/#342) und **B** (`cert-monitor-wiring.ts` + Test, #297/#344) sind **code-verifiziert komplett** (im Baum per grep/ls bestätigt, nicht nur Commit-Prosa). Vier offene Gates: **G1** (D3-Intermediate-Laufzeit 1–3 J → Christian → ADR-045 `Accepted` → Runbook), **G2** (C-Klassifikation Blocker/Fast-Follow + Form + Distribution → CO/Owner → `crl.ts`-Verdrahtung), **G3** (Cross-Vendor-CO infra-blockiert, `codex`/`agy` NOT in PATH), **G4** (TL-14b ⛔). **Kein non-gated Slice offen:** Runbook-Volltext ist per Prozess nach G1 sequenziert, C-Verdrahtung hängt an G2, A2-rest bis TL-14b deferred ⇒ Vorziehen = Nebelmaschine (PR-#83-Lehre). `crl.ts:5–7`-Header-Falschtext dokumentiert (Reparatur gehört in den G2-Slice, nicht in Churn-Micro-PR). `TODO.md`-Kopf um GATE-STATUS-Zeiger ergänzt. **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** Review-of-Record über **claude/agy** (nie Codex/MiniMax/pal:chat) + Self-CR. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, alle offenen Gates (TL-08/09/10, C/D3/Runbook/TL-14b). Kein Deploy/Secret/State, Risiko-Delta **null**. |

**Typ:** Doc-only Post-Merge-Reconcile (PR-Nummern #346 + #347 + TODO-Aufholung #338→#347 — kein Code/Test/Design). Kein Deploy/Secret/Host. **DO:** `COMPLIANCE-TABLE.md`, `CHANGES.md`, `TODO.md`, `changes/2026-07-28_reconcile-pr-346-347.md`, dieser Eintrag.

| (offen, base=main) | 2026-07-29 07:40 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only (TL-14a: G1-Entscheidungs-Brief für Christian) → CO/CG/TS entfallen** (kein Code/Test-Diff, **kein Beschluss**, kein Gate verschoben, **kein Fortschritt**; konsolidiert bereits gegroundetes Material für den Owner-Sign-off; Präzedenz doc-only #345/#346/#347/#349; `clink`/`gemini`/`codex`/`agy` nicht im PATH). **Der Bedarf:** die G1-Entscheidung (D3-Intermediate-Laufzeit), die ADR-045 auf `Proposed` hält, lag verstreut über ADR-045 §D3/§130 + `TL-14a-consensus-result-D1-D6.md`; die ältere `TL-14a-decision-checklist.md` widersprach ihr sogar noch (D3 „≥5 J", vor-Consensus). **Was:** (1) neue `docs/architecture/TL-14a-G1-decision-brief.md` — die **eine** Owner-Entscheidung (exakte D3-Zahl im Korridor **1–3 J**; opus ~12–24 Mon. vs. sonnet 3 J, beide verwerfen ≥5 J) + die mit-zu-bestätigenden Konsens-Punkte D1/D2/D4/D5/D6 + Fill-in-Sign-off-Block; explizit gehalten, dass der Brief **nichts** entriegelt und die Code-Lane nach dem Sign-off weiter an **G2** (Revocation/`crl.ts`) und **G4** (TL-14b ⛔) gated bleibt; Vorbedingung B (`cert-monitor-wiring.ts`) als erfüllte Absicherung der längeren-Laufzeit-Option benannt. (2) `TL-14a-decision-checklist.md` D3-Zeile als **überholt** markiert + Supersession-Hinweis (Ehrlichkeit, kein neuer Beschluss). **TS:** kein `.ts`-Diff, Suite unverändert **2101 grün**. **CR:** Review-of-Record über **claude** (Self-CR; Faktentreue gegen ADR-045 + Consensus-Protokoll, Gating explizit; `agy`/`codex` nicht im PATH). **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-046 §9, msg 1453, alle offenen Gates (TL-08/09/10, G2/C, G3, G4/TL-14b). Kein Deploy/Secret/State, Risiko-Delta **null**. |
| #352 | (base=main, gemergt) | 2026-08-25 16:45 | ✅ | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only (TL-14a: G2-Consensus-Ergebnis zu Auflage C) — CO ist hier der Inhalt selbst, CG/TS entfallen** (kein Code/Test-Diff, **keine Owner-Entscheidung**, kein Gate entriegelt; Präzedenz doc-only #345/#346/#347/#349/#351; Muster `TL-14a-consensus-result-D1-D6.md`; `clink`/`gemini`/`codex`/`agy` nicht im PATH). **Der Bedarf:** Gate **G2** (C-Klassifikation) war die einzige verbleibende Nicht-Owner-Entriegelung; der in `TL-14a-blocker-C-grounding.md` §2 aufgedeckte Widerspruch (Consensus stuft **A–C** blockierend, ADR-045 §74 führt nur **A/B**) stand seit 2026-07-27 unentschieden. **Was:** neue `docs/architecture/TL-14a-consensus-result-C.md` protokolliert einen tatsächlich gelaufenen `pal:consensus`-**Doppellauf**. **Ergebnis — C wird gesplittet** statt „Blocker" *oder* „Fast-Follow" (der Streit lag an einem falsch geschnittenen Objekt): **C1** = lokales `isRevoked`-Enforcement (App-Ebene, `agent-card.ts:311` Issuer **+** `:353` Leaf, bestehende `crl.ts` verdrahten statt Neubau) ⇒ **blockierend vor TL-14b, aber klein**; **C2** = Mesh-Verteilung der Denylist ⇒ **Fast-Follow, bewusst nichts bauen** (`crl.json` bleibt lokal + owner-gepflegt; eine fernverteilte unsignierte Denylist wäre ein **DoS-Primitiv**, eine signierte kostet mehr Angriffsfläche als sie löst; Trigger >25 Nodes / Nicht-Owner-Betreiber dokumentiert). ⇒ **Der Consensus meinte C1, die ADR meinte C2** — beide hatten für ihren Anteil recht. **Form:** Fingerprint-**Denylist**, kein CRL/OCSP (OCSP = SPOF im Trust-Pfad, soft-fail wirkungslos/hard-fail mesh-weiter Ausfall; eine echte CRL bräuchte für **jede** Sperrung den **Offline-Root-Key** ⇒ direkter Widerspruch zu D3). **Enforcement-Punkt empirisch erzwungen:** `tls-transport-pathlen.conformance.test.ts` (#342) belegt, dass Node-TLS `pathLen` nicht durchsetzt ⇒ Trust sitzt App-seitig, Revocation gehört an denselben Punkt (ein Entscheidungspunkt statt zwei divergierender). **Stärkster Einzelgrund für C1** (von der Fast-Follow-Seite eingeräumt): die kalte **TH02-Reserve (D6, ADR-045 §68-72)** ist ohne Sperrfähigkeit nur **halb** aktivierbar — sie stellt **Verfügbarkeit** wieder her, **nicht Integrität**, weil das kompromittierte Intermediate gültig bleibt („unsperrbare Reserve = halbe Reserve"). **Auflagen für den späteren C1-Slice** benannt: Regression-Tests (revozierter Leaf abgelehnt / revoziertes Intermediate ⇒ ganze Kette abgelehnt), **Kollisions-Test „Alt-Pin aktiv (D4-Doppel-Pin) + Alt-Intermediate revoziert"** (ohne ihn sperrt die Denylist den Cutover **selbst aus**), eigener **Audit-Event** für Ablehnungen (sonst greift Revocation unsichtbar — Fehlerklasse #272/#278), Korrektur des falschen `crl.ts:5-7`-Headers **im selben Slice**. **Lauf-Ehrlichkeit:** Runde 1 lieferte nur **eine** Stimme — `cli-codex-gpt` und `cli-agy-gemini-pro` fielen **beide** mit *"executable not found in PATH"* aus (PAL *listet* sie als konfiguriert, sie scheitern beim Aufruf) ⇒ **G3 am 2026-08-25 erneut laufzeit-verifiziert offen**; deshalb Runde 2 **adversarial nachgezogen** (`cli-claude-sonnet` *for* = blockierend vs. `cli-claude-opus` *against* = Fast-Follow, mit der These im Volltext als Angriffsziel). Der Gegenangriff **traf** (Blast-Radius am falschen Objekt verglichen: „Root vorher/nachher" statt „heißer Signierschlüssel vorher/nachher"; plus **D5-Recovery-Zirkelschluss** — weil D5 den `ca.crt.pem`-Swap ablehnt, ist die einzige Recovery derselbe mehrtägige gated Re-Onboard-Sweep) und korrigierte die **Begründung**, nicht das Ergebnis. **Ergebnis-Charakter: Same-Vendor-2-Modell-Panel** (opus 8/10, sonnet 7/10), **kein Cross-Vendor-Pass** — im Dokument ausdrücklich so gekennzeichnet. **Rückwirkung auf G1:** eine kürzere D3-Laufzeit ist **kein Revocation-Ersatz** (deckelt nur planmäßige Rotation, nichts gegen „Kompromittierung an Tag 2"); Empfehlung **24 Monate**, begründet als **Zeremonie-Probe-Erzwingung** (D6) — **Korridor 1–3 J unverändert, die Zahl bleibt Christians**. **Nachgezogen (keine Beschlüsse):** `TL-14a-G1-decision-brief.md` §1-Nachtrag + §3-Stand; `ADR-045` §100 Zeiger (**Status/§74 bewusst NICHT geändert** — folgt erst der Ratifizierung); `TL-14a-gate-status.md` §2/§3/§5 (G2 = „CO erledigt, Owner offen"; beide Rest-Entriegelungen jetzt **reine Owner-Entscheidungen**). **Offen gelassen:** Fill-in-Ratifizierungs-Tabelle für die **Owner-Hälfte** von G2; der C1-Slice ist **nicht freigegeben**. **Vorbehalte im Dokument geführt:** Aufwandsschätzung („~1 Tag") **nicht** code-verifiziert; Fingerprint-**Format** an `:311`/`:353` ggü. `crl.ts`-Erwartung ungeprüft (ggf. Normalisierung). **TS:** kein `.ts`-Diff; **Daemon-Suite 2101 grün** (`npx vitest run --root packages/daemon`, unverändert). Root-`npm test`: **2222 Tests grün**, 1 Test-**File** meldet `failed` — `scripts/check-native-modules.test.cjs` („No test suite found"), per `git diff main` **unverändert gegenüber `main`** ⇒ **vorbestehendes Vitest-Sammel-Artefakt, nicht durch diesen PR verursacht**. **CR:** Review-of-Record über **claude** (Self-CR; `agy`/`codex` nicht im PATH) — Faktentreue der Zitate gegen `crl.ts`, ADR-045 §68-72/§74/§90-94/§100 und die C-Grounding-Note geprüft, Gating-Sprache explizit. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** ADR-045 §Status/§74, ADR-046 §9, msg 1453, alle offenen Gates (TL-08/09/10, **G1**, **G2-Owner**, G3, G4/TL-14b). Kein Deploy/Secret/State, Risiko-Delta **null**. |
| #353 | (base=main, gemergt) | 2026-08-26 06:15 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Beschluss-Eintragung (TL-14a: Owner-Sign-off G1 + G2) — CO/CG/TS entfallen** (kein Code/Test-Diff; **trägt** einen Owner-Beschluss ein, **trifft** keinen; die inhaltliche CO-Grundlage ist der Lauf vom 2026-08-25, **#352**; `clink`/`gemini`/`codex`/`agy` nicht im PATH). **Der Beschluss (Christian, 2026-08-26):** **G1 — D3 = 24 Monate** (+ D1/D4/D5/D6 mit-bestätigt; **D2 = CO/technisch**, bereits per Consensus entschieden und **nicht** Teil der Sign-off-Tabelle) und **G2 — Auflage C ratifiziert** (**C1** blockierend, **C2** Fast-Follow). **Was eingetragen wurde:** (1) **`ADR-045` §Status `Proposed` → `Accepted`**; (2) **§D3** von „OFFEN (Owner-Entscheidung)" auf **beschlossen: 24 Monate** — Begründung bewusst **Zeremonie-Probe-Erzwingung (D6)**, **nicht** Kompromittierungs-Fenster-Kompensation (der G2-Consensus hatte belegt: kürzere Laufzeit ist **kein** Revocation-Ersatz, sie deckelt nur planmäßige Rotation); Root-Laufzeit Korridor 10–15 J bestätigt, exakte Zahl bei der Zeremonie; (3) **neue §Vorbedingung C1 (blockierend)** — Befund (`crl.ts` = fertige, aber 0-Aufrufer-Denylist), tragender Grund (**D6-Reserve ohne Sperrfähigkeit = halbe Reserve**: Verfügbarkeit ja, **Integrität nein**; verschärft durch D5, das den Chain-Swap ablehnt ⇒ einzige Alternativ-Recovery = mehrtägiger gated Sweep), Form **Denylist** statt CRL/OCSP (OCSP = SPOF im Trust-Pfad; echte CRL bräuchte für jede Sperrung den **Offline-Root-Key** ⇒ Widerspruch zu D3), Enforcement **App-Ebene** (empirisch erzwungen durch **#342**: Node-TLS setzt `pathLen` nicht durch), **beide** Fingerprints (`agent-card.ts:311` Issuer **+** `:353` Leaf — nur Leaf wäre halber Fix, deckt den D6-Fall nicht), **3 Pflicht-Tests** inkl. **„Alt-Pin aktiv (D4-Doppel-Pin) + Alt-Intermediate revoziert"** (sonst sperrt die Denylist den Cutover **selbst aus**), eigener **Audit-Event** (sonst greift Revocation unsichtbar — Fehlerklasse #272/#278), `crl.ts:5-7`-Header **wahr machen**, plus offener **Format-Vorbehalt** (`sha256:`-Präfix/Trennung ggf. Normalisierung; „~1 Tag" **nicht** code-verifiziert); (4) **§Konsequenzen: C2 als bewusst NICHT gebaut** dokumentiert (lokale owner-gepflegte `crl.json`, kein Schreib-Endpunkt/Gossip; unsignierte Fernverteilung = **DoS-Primitiv**, signierte = mehr Angriffsfläche als Nutzen) **inkl. Re-Evaluierungs-Trigger** (>25 Nodes / Nicht-Owner-Betreiber); der **Klassifikations-Hinweis vom 2026-07-27 ist aufgelöst** (*der Consensus meinte C1, die ADR meinte C2* — beide hatten für ihren Anteil recht); (5) **§Nächste Schritte** neu sequenziert (1+2 abgehakt, **C1 als Schritt 3**, Runbook **entriegelt**, TL-14b setzt zusätzlich C1 voraus). **Nachgezogen:** `TL-14a-G1-decision-brief.md` §5 Sign-off-Tabelle **ausgefüllt** (7 Zeilen) + G1 geschlossen; `TL-14a-consensus-result-C.md` Ratifizierungs-Tabelle **ausgefüllt** (4 × ja); `TL-14a-gate-status.md` G1+G2 **GESCHLOSSEN** + Nachtrag (Runbook entriegelt/agent-ausführbar, C1 definiert aber nicht freigegeben, G3/G4 weiter gated); `TL-14a-decision-checklist.md` alle sechs D-Zeilen ⬜/🟨 → **✅ beschlossen** + Register als **historisch** markiert. **Interpretations-Grenze (bewusst eng):** Christians Wortlaut war „D3 = 24 Monate, C1/C2 ratifiziert — trag es ein"; **abgeleitet** (weil der G1-Brief den Sign-off genau so definiert): D1/D4/D5/D6 + Statuswechsel (**D2 ausgenommen** — CO/technisch). **NICHT abgeleitet, sichtbar offen gelassen:** (a) die **Freigabe des C1-Umsetzungs-Slices** — Klassifikation ≠ Code-Freigabe, eigener Owner-Akt; (b) die **exakte Root-CA-Laufzeit** — nie genannt, Korridor-Führung beibehalten. **Reconcile mit-erledigt:** #352-Erst-Spalte `(offen, base=main)` → `#352 | (base=main, gemergt)` + CHANGES-Überschrift `, #352)` (`gh`-verifiziert MERGED, `de77a2f`, 2026-08-25T16:35:34Z). **TS:** kein `.ts`-Diff, Daemon-Suite unverändert **2101 grün**. **CR:** ✅ **ECHTES Fremd-Vendor-Review über `agy` (Gemini)**, kein Self-CR — **Verdikt APPROVE** mit 1 MEDIUM + 1 NIT. **MEDIUM behoben:** „D1/**D2**/D4/D5/D6 mit-bestätigt" ordnete **D2** fälschlich dem Owner-Sign-off zu; D2 (`pathLen 0`) ist **CO/technisch** und stand **nicht** in der Sign-off-Tabelle ⇒ in allen betroffenen Dateien korrigiert (Checklist-D2-Zeile jetzt „per Consensus 2026-07-19"). **NIT bewusst offen:** überholter KW31-Block in `TODO.md` bleibt markiert stehen (Historie sichtbar; laut agy „kein echter fachlicher Widerspruch"). Faktentreue + Ehrlichkeit von agy als **PASS** bewertet. **⚠️ G3-KORREKTUR:** `codex`/`agy` sind **vorhanden in `~/.local/bin`**, nur **nicht im PATH** — `pal` scheitert deshalb, ein **direkter Aufruf über absoluten Pfad funktioniert**; G3 ist ein **PATH-Problem, kein Verfügbarkeitsproblem**, und für Reviews ab sofort umgehbar. **PC:** Secret-Scan clean (nur Doku). **Nicht berührt:** Code/Config/Deploy/Secret/State, **G3** (Cross-Vendor-CO), **G4** (TL-14b ⛔), ADR-046 §9, msg 1453, TL-08/09/10. Risiko-Delta **null**. |
| (offen, base=main) | (base=main) | 2026-08-26 07:50 | n/a | n/a | ✅ | ✅ | ✅ | ✅ | **BLOCKER-Befund + Regressionsschutz (TL-14: ADR-045 D2 `Root pathLen 0` unvereinbar mit der Zweistufen-Hierarchie) — CO/CG entfallen** (**kein** Design-Vorschlag, ein **Faktenbefund**; die Korrektur-Entscheidung ist ausdrücklich **offen gelassen**). **Auftrag → Ergebnis:** Auftrag war der durch den G1/G2-Sign-off (#353) entriegelte Slice **„Runbook-Volltext + Zeremonie-Skripte"**. Beim Schreiben von **Schritt 2 von 7** (Offline-Wurzel-Zeremonie) — der Zeile `basicConstraints = critical, CA:TRUE, pathlen:<N>` — zeigte sich, dass der von **D2** vorgeschriebene Wert die Zielhierarchie **unbrauchbar** macht. Slice **gestoppt und gemeldet** statt fortgeschrieben (ADR-045 verlangt selbst, dass Skripte nicht „an genau der offenen Stelle unvollständig" entstehen; `gate-status.md` §3 „Nebelmaschine"). **Der Befund:** RFC 5280 §4.2.1.9 — `pathLenConstraint` = max. Anzahl nicht-selbst-ausgestellter Zwischen-CAs, die dem Zertifikat im Pfad **folgen** dürfen ⇒ `Root pathlen:0` erlaubt **kein einziges Intermediate**; **jedes** Node-Cert der neuen Kette wäre mesh-weit ungültig. Das **Schutzziel** der ADR ist richtig, nur an der **falschen Stufe kodiert**: dass TH01/TH02 keine Sub-CAs ausstellen dürfen, erzwingt der `pathLen 0` **am Intermediate** — die Verwerfung von „`pathLen 1`" unter §Verworfene Alternativen („TH02 könnte Sub-CAs") ist eine **Fehllesung**. **Korrekt: Root `pathLen 1` + Intermediate `pathLen 0`.** **Dreifacher, unabhängiger Beleg:** (1) **der eigene grüne Testbestand widerspricht D2 bereits** — `chain-verify.test.ts:55` baut die **funktionierende** Kette mit **Root `pathLen 1`**, `:61-67` nagelt `Root pathLen 0` explizit als **Ablehnung** fest (seit Vorbedingung A #298/#311 im Repo, nur nie gegen D2 gehalten); (2) **OpenSSL 3.0.13, vendor-neutral** (kein node-forge-Artefakt): `pathlen:0` ⇒ *„error 25 at 2 depth lookup: path length constraint exceeded"*, `pathlen:1` ⇒ `OK` — einziger Unterschied ist der Root-`pathLen` (dasselbe Intermediate-Keypair/dieselbe CSR von beiden Roots signiert), re-runnable via neuem `scripts/tl14-ca/tl14-pathlen-proof.sh` (nur `mktemp -d`, **kein** Daemon-State, Exit 0 = reproduziert / Exit 1 = Note überholt); (3) **neuer End-to-End-Profiltest** `tests/integration/tl14-ceremony-cert-profile.test.ts` (**+4 Tests, grün**), der eine **reale Lücke** schliesst: **alle** bisherigen Ketten-Tests minten ihre Certs mit **node-forge** — also mit demselben Werkzeug, das sie prüfen —, während die Zeremonie auf dem Air-Gap-Rechner mit **OpenSSL** läuft; ob ein **OpenSSL-erzeugtes** Profil vom Daemon akzeptiert wird, war **ungetestet** und wäre erst im **TL-14b-Fenster** aufgefallen. Der Test baut die Kette wie das Zeremonie-Skript (echte CSR-/Signatur-Schritte, SPIFFE-SAN, **730 Tage = D3-Beschluss**) und prüft mit der **echten** `verifyPeerCertChain`: Root p1+Inter p0 ⇒ **akzeptiert** · Root p0 (D2-wie-geschrieben) ⇒ **abgelehnt** · Sub-CA unter dem Intermediate ⇒ **abgelehnt** (⇒ **die Korrektur schwächt D2 NICHT**) · **`certFingerprint()` == OpenSSL-DER-SHA256** lowercase hex (⇒ **D4-Doppel-Pin-Kompatibilität belegt** — zweites ungetestetes Risiko: bei abweichenden Formaten hätte der Cutover **ins Leere gepinnt**). **Neu:** `docs/architecture/TL-14a-D2-pathlen-blocker.md` (Norm-Zitat, Herleitung des Denkfehlers, dreifacher Beleg, **drei Entscheidungs-Optionen** A/B/C mit Empfehlung **A**, plus §6 „Was der Befund NICHT ändert"), `scripts/tl14-ca/tl14-pathlen-proof.sh`, der Profiltest. **ADR-045 §D2** erhält einen ⛔-**Sichtbarkeits**-Hinweis — **die Entscheidung selbst ist NICHT geändert** (Owner-/CO-Akt), Muster wie der Klassifikations-Hinweis von #347. **TS:** ✅ **+4 Integrationstests grün**; Daemon-Suite unverändert **2101 grün**, Gesamt **2226 grün**; Proof-Skript manuell verifiziert (Exit 0). **CR:** ✅ **zwei echte Fremd-Vendor-Reviews über `agy` (Gemini)** — kein Self-CR (Pfad via `~/.local/bin`, s. G3-Korrektur in #353). **Runde 1 (Befund-Diff): APPROVE, keine Findings** — RFC-5280-Auslegung eigenständig verifiziert („100% korrekt"), Stoppen des Slices als „einzig saubere Option" bestätigt (stilles `pathlen:1` = „Stealth-Fix"), Profiltest als „kein Theater" eingestuft. **Runde 2 (Korrektur-Diff): REQUEST_CHANGES — 2 HIGH + 1 MEDIUM; die 2 HIGH widerlegt, das MEDIUM bewusst abgelehnt** (ehrlich dokumentiert statt weggelassen): **HIGH-1/2** behaupteten, `tl14-pathlen-proof.sh` baue „weiterhin nur eine Kette mit `Root pathLen 0`" und melde fälschlich Erfolg — **falsch**, das Skript iteriert `for PL in 0 1`, baut **beide** Ketten und prüft **beide** Erwartungen; Ursache des Fehlalarms war, dass die Prüfschleife in der **unveränderten Dateimitte** steht und deshalb **nicht im Diff** war. **Empirisch widerlegt per Mutationstest**: baut man beide Roots mit `pathlen:0`, schlägt das Skript korrekt an (Ausgabe „ABWEICHUNG … NICHT mit der Zeremonie fortfahren", **Exit 1**) ⇒ keine Änderung nötig. **MEDIUM-3** (Datei in `tl14-pre-ceremony-check.sh` umbenennen) **abgelehnt**: Auftrag war ausdrücklich „nur die minimale Textkorrektur für diesen Blocker"; das Skript **ist** weiterhin ein Beleg (beweist beide Richtungen der `pathLen`-Semantik); eine Umbenennung zöge Referenzen in **sechs** Dateien nach ⇒ genau der Churn, den die Minimalitäts-Vorgabe vermeiden soll. Nachholbar als eigener Hygiene-Slice. **Runde 3 (Re-Review am VOLLTEXT statt am Diff): APPROVE, keine HIGH/MEDIUM** — der Reviewer verifizierte die Schleife selbst und stufte die Runde-2-HIGHs ausdrücklich als **„eindeutig Fehlalarme"** ein („Wer nur den Diff liest, verpasst den Kontext"); zusätzlich bestätigt: **keine** normative `Root pathLen 0`-Stelle mehr in ADR-045, Profiltest „hervorragend und aussagekräftig", Blocker-Note „faktisch zu 100 % korrekt". **1 LOW übernommen (echter latenter Bug):** `grep … | head -1` ist unter `pipefail` eine SIGPIPE-Falle (grep→141, `set -e` bricht ab; greift heute nur nicht, weil `openssl verify` genau eine error-Zeile liefert) ⇒ auf **`grep -i -m 1 … || true`** umgestellt. Der Reviewer hielt das für optional; umgesetzt trotzdem, weil das Skript als **Vor-Zeremonie-Check auf einer Air-Gap-Maschine** läuft, wo ein spuriöser Abbruch als „OpenSSL verhält sich anders" fehlgedeutet würde. **Verifiziert:** Normallauf Exit 0 · Mutationstest Exit 1 · Mehrzeilen-Fall korrekt ohne Abbruch. **PC:** Secret-Scan clean; **kein `packages/`-Diff** verifiziert. **Reconcile mit-erledigt:** #353-Erst-Spalte → `#353 | (base=main, gemergt)` + CHANGES-Überschrift `, #353)` (`gh`-verifiziert MERGED, `63cd72f`). **Nicht berührt:** `packages/daemon/**` (`verifyPeerCertChain` verhält sich **korrekt** und lehnt zu Recht ab — **kein** Fix nötig), D1/D3/D4/D5/D6, C1/C2, G1-Sign-off, ADR-045-Status (`Accepted` bleibt). **C1-Slice weiterhin NICHT freigegeben**, TL-14b weiter ⛔. Kein Deploy/Secret/State, Risiko-Delta **null**. |

---

*Letzte Aktualisierung: 2026-07-28 06:15 — docs(reconcile): PR-Nummern-Nachtrag für #346 + #347 + TODO-Aufholung #338→#347 — die beiden jüngsten Merges blieben offen (#346 `923142f` = der #341–#345-Reconcile selbst, #347 `0345546` = ADR-045-Klassifikations-Hinweis); COMPLIANCE-Erst-Spalten `(offen, base=main)` → `#NNN` + `(base=main, gemergt)` (08:42→#346, 09:45→#347), CHANGES-Überschriften markiert, `gh`-verifiziert MERGED; zusätzlich `TODO.md` von #338 auf #347 aufgeholt (substantielle Task-Zeilen #341/#342/#343/#344/#347 mit Merge-Nummer; reine Reconcile-PRs #340/#345/#346 ohne TODO-Eintrag) ⇒ COMPLIANCE/CHANGES/TODO spiegeln dieselben Nummern bis #347; doc-only, kein `.ts`-Diff, Suite unverändert 2101 grün. Zuvor: docs(tl14a): ADR-045-Klassifikations-Hinweis zu Auflage C — der #341-Befund (Consensus stuft A–C blockierend, ADR-045 führt nur A/B und C in §100 stumm als „out of scope") stand bislang nur extern; neuer Hinweis in ADR-045 §100 macht die nicht-owner-ratifizierte Herabstufung sichtbar + verweist auf die C-Grounding-§2, entscheidet nichts; sauberer TL-14-Befund, doc-only, gate-frei, Suite unverändert 2101 grün. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #341 … #345 (Dispatch nannte #343-#345, zusätzlich #341/#342 offen gefunden → alle fünf; Erst-Spalten → echte PR-Nummern, CHANGES-Überschriften markiert; `gh`-verifiziert MERGED `dd2c4e9`/`873c2d2`/`ba9a143`/`f52af5d`/`14b90ad`); doc-only, kein `.ts`-Diff, Suite unverändert 2101 grün. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #340 (laut Reconcile-Bericht 2026-07-27 03:30) — #340-Erst-Spalte `(offen, base=main)` → `#340`, CHANGES-Überschrift `…, #340)`; reine Nachpflege, `gh`-verifiziert MERGED (`53dfa2f`), kein `.ts`-Diff, Suite unverändert 2101 grün. Zuvor: refactor+test(tl14a): Vorbedingung-B-Monitor-Verdrahtung regressionsfest (`cert-monitor-wiring.ts` `buildCertExpiryMonitorSpecs` — Node/CA getrennte Quellen+subjects; `index.ts` mapt darüber, verhaltensbewahrend) — schließt die ungetestete Verdrahtungslücke (KEIN Doppel zu #297); +4 Tests, Suite 2101 grün ⇒ B code-seitig komplett. Zuvor: 2026-07-27 07:50 — docs(tl12): B1-Reihenfolge-Pflicht — Keyid-Backfill VOR der `UNIQUE`-Bedingung; Befund aus dem #323-Review: `signer_keyid` wird heute schon mit dem PEM-Hash befüllt + über `idx_messages_order` indiziert, deshalb müssen Altzeilen (kanonisch aus dem unveränderlichen `signer_pubkey`) gebackfillt werden, bevor B1 die `UNIQUE`-Bedingung setzt — sonst Übergangsloch im Replay-Schutz; reine TODO-Notiz, kein Schema/Migration, Slice B bleibt gated. Zuvor: test(tl14a): Vorbedingung A — pathLen auf der Transport-Ebene (`tls-transport-pathlen.conformance.test.ts`, echter mTLS-Handshake) — BEFUND: Node-TLS erzwingt pathLen NICHT (Root-pathLen0+Intermediate akzeptiert; App-Ebene `verifyPeerCertChain` lehnt ab, Kontrast im selben Test) ⇒ Zwei-Stufen-Trust muss über die App-Ebene laufen; Vorbedingung A code-seitig komplett; +4 Tests, non-gated (D3-unabhängig), Suite 2097 grün. Zuvor: docs+test(tl14a): Auflage C (Revocation) gegroundet + `crl.ts`-Charakterisierung — C war die einzige der drei blockierenden Consensus-Auflagen ohne Grounding; Befund: `crl.ts` ist bereits die vorgeschlagene Fingerprint-Denylist, aber 0-Aufrufer-Dead-Code + ungetestet (Kopf behauptet Verdrahtung, die fehlt); Widerspruch aufgedeckt (Consensus A–C blockierend vs ADR-045 nur A/B); +6 Charakterisierungs-Tests, keine Verdrahtung/Entscheidung, Suite 2093 grün. Zuvor: docs(reconcile): #339-Nachtrag + TODO-PR-Nummern-Aufholung (#331→#339) — #339-Erst-Spalte/CHANGES-Überschrift nachgezogen; `TODO.md` (hing >5 PRs zurück) mit den Merge-Nummern #331/#333/#334/#335/#337/#338 aufgeholt, sodass COMPLIANCE/CHANGES/TODO dieselben Nummern spiegeln; doc-only, kein `.ts`-Diff, Suite unverändert 2087 grün. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #336 + #337 + #338 (Erst-Spalten → echte PR-Nummern, CHANGES-Überschriften markiert; `gh`-verifiziert MERGED); doc-only, kein `.ts`-Diff, Suite unverändert 2087 grün. Zuvor: feat(tl10): reiner TOML-Text→Matrix-Loader `parseFreigabeMatrixToml` (D1-Prep) — mechanischer Format-Schritt (kein fs, 0 Aufrufer, fail-closed via `parseFreigabeMatrix`); die kuratierte Policy-Datei + Verdrahtung + D3-Sign-off bleiben gated; gate-neutral wie #317/#319, +7 Tests, Suite 2087 grün. Zuvor: fix(tl10): zwei #300/#319-CR-Altlasten im Slice-A-Parser (`freigabe-matrix.ts`) — whitespace-only Kanalname wird jetzt abgelehnt (`trim()`), `resolveEntry` gibt den `decider` als frische Kopie (`cloneDecider`) statt per Referenz; reine Parser-Korrektheit, **0 Runtime-Aufrufer** (kein Gate-Vorgriff), +6 Regressionstests, mutations-verifiziert, Suite 2080 grün. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #332 + #333 + #334 + #335 (Erst-Spalten → echte PR-Nummern, CHANGES-Überschriften markiert; `gh`-verifiziert MERGED); doc-only, kein `.ts`-Diff, Suite unverändert 2074 grün. Zuvor: test(tl12): S5 Read-Surface am HTTP-Rand gehärtet (`tl12-order-read-surface.test.ts`) — `GET /api/inbox` re-verifiziert signierte Aufträge **live**; echte On-Disk-Manipulation (zweite SQLite-Verbindung) → `VALID→INVALID`, `200` bleibt; fail-closed-Loop bewacht; +3 Tests, mutations-verifiziert, Suite 2074 grün; test-only, S6 owner-gated unberührt. Zuvor: docs(tl11): §6-Referenz-Konsument an die getesteten Primitive (`interpretWakeFrame`/`coldStartSweepDecision`) gebunden — die kopierbare Referenz nutzt jetzt den getesteten Kern (#331) statt hand-ausgeschriebenem `ev.data?.reason`-Parsing; der Supervisor schreibt nur noch Transport + `pokeCli`; keine Verhaltensänderung, doc-only, Suite unverändert 2071 grün. Zuvor: test(tl11): Emit→Decision-Brücke (`tl11-wake-emit-to-decision.test.ts`) — verbindet die beiden Enden, die die drei bestehenden Wake-Test-Ebenen offen ließen: socket-frei `inbox:new` → echter Emitter → echter `MeshEventBus` → dieselbe Serialisierung wie der Draht (`websocket.ts:266`) → `interpretWakeFrame`; beweist, dass der Kern (#331) korrekt über den tatsächlich emittierten Frame entscheidet; +6 Tests, mutations-verifiziert, Suite 2071 grün; test-only, de-riskt Slice B weiter. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #330 + #331 (dieselbe Selbst-Reconcile-Lücke; der #328-Nachtrag war bereits durch #330 erledigt → hier die beiden nächsten Zeilen #330/#331); COMPLIANCE-Erst-Spalten → #330/#331, CHANGES-Überschriften markiert; `gh`-verifiziert MERGED; doc-only, kein `.ts`-Diff, Suite unverändert 2065 grün. Zuvor: feat(tl11): konsumentenseitiger Wake-Kern `interpretWakeFrame` (rein, 0 Aufrufer, kein Runtime-Delta) — zieht den nicht-transportgebundenen Teil des Consumer-Contract-§6-Pseudocodes als getesteten Kern heraus (Payload nur unter `.data`, §3/§4/§5-Semantik, wirft nie); pinnt konsumentenseitig die `ev.reason`-statt-`ev.data.reason`-Fehlklasse (#282); +20 Tests, mutations-verifiziert, Suite 2065 grün; de-riskt Slice B, entfernt den Blocker nicht. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #328 + #329 (dieselbe Selbst-Reconcile-Lücke wie #327; der #327-Nachtrag selbst war bereits durch #328 erledigt → hier die nächste Stufe); COMPLIANCE-Erst-Spalten → #328/#329, CHANGES-Überschriften markiert; `gh`-verifiziert MERGED; doc-only, kein `.ts`-Diff, Suite unverändert 2045 grün. Zuvor: docs(runbook): TL-11-Runbook §5.1 nennt den optionalen Daemon-Sweep `TLMCP_WAKE_SWEEP_ENABLED` (Default AUS, ergänzt die Cold-Start-Pflicht, ersetzt sie nicht); doc-only, kein Flag-Flip, Suite unverändert 2045 grün. Zuvor: docs(reconcile): PR-Nummern-Nachtrag für #327 (Selbst-Reconcile, den #327 nicht falten konnte); danach 0 stale Erst-Spalten mit PR-Bezug; doc-only, Suite unverändert 2045 grün. Zuvor: docs(arch): TL-12 Zustellpfad end-to-end (`TL-12-delivery-path.md`) — S1–S5 gebaut, **S6 Abarbeitung nicht gebaut und owner-gated**; TL-08/09/10 je Station verortet statt weggelassen; alle zehn Anker gegen c4b5261 verifiziert; doc-only, Suite unverändert 2045 grün. Zuvor: feat(tl11): Reconciliation-Sweep verdrahtet (`sweep-wiring.ts`, Auslöser `agentRegistry.on('register')`, eigener Coalescer, **Default AUS** via `TLMCP_WAKE_SWEEP_ENABLED`) — schließt die Reconnect-Lücke des best-effort-Wake; +18 Tests, Suite 2045 grün; agy-CR: 1 HIGH (O(N²)-Hazard) behoben; Slice B und der Flag-Flip bleiben gated. Zuvor: docs(arch): ADR-046 Rev. 2 — Implementierungs-Anker gegen `e994e65` verifiziert (drei `index.ts`-Anker der Erstfassung verschoben, darunter der V1-tragende default-Drop), Producer gepinnt, dritter Card-Pfad benannt; neue §6 fail-closed-Grenzen (inkl. „Feature-Advertisement ist kein Trust-Grant“), §7 Seed-Flag-Semantik, §8 CO-Grenze; doc-only, kein `protocol`-Block, kein ORDER-Flip, Suite unverändert 2027 grün; CR durch **echtes `agy`-Review** (aus `~/.local/bin`, nicht im PATH): 2 HIGH behoben — §6 war nicht CO-unabhängig, vierte Card-Parse-Stelle übersehen. Zuvor: feat(tl12): B1-Prep Reserve-vor-Dispatch/Commit-Vertrag als reine Zustandsmaschine (`order-ledger-protocol.ts`, 0 Aufrufer, keine Persistenz); erfüllt den §4-Auftrag, das Protokoll jetzt gemeinsam mit B3 zu spezifizieren; at-most-once erzwungen (`failed` terminal, kein Retry), `mayDispatch` einziger Auswertungspfad; +27 Tests, Suite 2027 grün; CR: kein HIGH, 3 MEDIUM (u.a. ein echter At-most-once-Bypass über den malformed-Zweig) an der Wurzel behoben; B1-Persistenz/B3/§9 bleiben gated; inkl. #323-Reconcile. Zuvor: fix(tl12): `canonicalOrderKeyId` (sha256 über DER-SPKI statt PEM-Text) — schließt die Format-Malleabilität, bevor der Keyid Ledger-Uniqueness/Revocation-Join-Key wird (Slice-B-Scoping §3); `orderKeyId` unverändert, 0 Aufrufer, +8 Tests inkl. Defekt-Beleg, Suite 2000 grün; Slice B/C bleiben gated; inkl. #322-Reconcile. Zuvor: feat(tl11): Reconciliation-Sweep-Kern `computeSweepTargets` (rein, fail-closed, 0 Aufrufer) + ADR-047-Scoping Rev. 2; das externe Review widerlegte die ursprüngliche „alles blockiert"-Einschätzung (HIGH-1) → Slice gebaut, Blocker-Behauptung zurückgezogen; §2-Begründung gegen ADR-043 CO-B korrigiert (HIGH-2: echter Blocker = 1→N-Amplifikation); +13 Tests, Suite 1992 grün; Verdrahtung/Slice B/TL-12 bleiben gated.*
