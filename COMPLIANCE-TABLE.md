...
| #        | PR    | Datum            | CO  | CG | TS | CR | PC | DO | Findings                           |
|----------|-------|------------------|-----|----|----|----|----|----|----|
| v0.34.30 | (#203, base=main) | 2026-06-25 14:35 | —   | —  | ⚠️ | ✅ | ✅ | ✅ | CR clink claude: Bash-Migration `rm`→`mv .disabled.<ts>` (reversibel); **Post-Review CR-MEDIUM** (`cleanup_existing` löschte Legacy-Agent → reinstall/update irreversibel) gefixt: cleanup löscht nicht mehr, Backup zentral in `install_macos_service`; **Post-Review CR-AMBER** (`cleanup_existing` prüfte `darwin` statt `macos` → macOS-Block toter Code) gefixt |
