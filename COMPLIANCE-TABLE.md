...
| 109 | tbd       | ADR-004 Phase 2 — Agent Registry REST API                        | 04-09 18:14 | —  | —  | ✅ | ✅ | ✅ | ✅ | **Dieser PR.** `agent-registry.ts` (in-memory Map, 3×HB Stale-Eviction, `maxEntries=1000` DoS-Guard, injectierbarer Clock + Timer-Shim) + `agent-api.ts` (4 loopback-only Fastify-Endpoints, strict Regex-Validation, 4-Komponenten-SPIFFE-URI-Building, 409/503/500/400 Error-Codes) + 4 neue Audit-Event-Types + `index.ts` Wire-up. 34/34 Tests gruen (19 Registry Unit + 15 API Integration via `fastify.inject()`). CO entfaellt (Design aus PR #84). CR Gemini Pro (0 HIGH/CRITICAL, 1× MEDIUM DoS-Cap + 2× LOW alle gefixt mit Regression-Tests). PC Gemini Pro (1× MEDIUM unregister-race analog zum heartbeat-Fix — gefixt). DO: ADR-004 Status-Update + CHANGES.md. |
