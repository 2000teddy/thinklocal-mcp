...
| (offen, base=main) | 2026-07-23 13:25 | n/a | n/a | n/a | ✅ | ✅ | ✅ | **Doc-only Scoping/Discovery (ADR-047 — TL-11 Wake-Nachlauf: die drei „Optional/danach"-Punkte) → CO/CG/TS entfallen** (die Note **trifft keine** Architektur-Entscheidung, sie bereitet sie vor — Präzedenz `TL-10-freigabe-matrix-scoping.md` §5 und ADR-046-Scoping; die drei Beschlüsse selbst bleiben CO-/owner-Sache; kein Code/Test-Diff; `clink`/`gemini` nicht im PATH). **Auftrag:** den kleinsten wirklich ungegateten Post-#320-Slice zu `WS-Instanz-Bindung` / `Opt-in-Broadcast-Wake` / `Reconciliation-Sweep` herausarbeiten. **Ehrliches Ergebnis: alle drei sind entscheidungsgebunden** — nicht aus Mangel an Mechanik, sondern weil jeder eine **gemergte, bewusst konservative Invariante ändert**; der kleinste ehrliche Slice ist daher die Note selbst statt eines Code-Slices, der eine offene Entscheidung vorwegnimmt (die drei Stichworte standen bisher **ohne** Code-Grounding, **ohne** benannte Entscheidungen, **ohne** Zerlegung in der TODO). **Nicht-Offensichtliches, das die Note aufdeckt:** (1) `agentFilter` hat **Doppelfunktion** — gerichtetes Routing (`websocket.ts:87-91`) **und** `from`/`to`/`agentId`/`peer_id`-Filter nicht-gerichteter Events (`:94-101`) → eine naive „Filter = eigene Identität"-Bindung wäre auch eine **unbeabsichtigte Feature-Rücknahme** für Dashboard-/CLI-Beobachter; (2) die **Client-Cert-Identität existiert im WS-Handler gar nicht** (kein `getPeerCertificate`, `ClientState` kennt nur `isLoopback` aus `req.ip`; mTLS liegt im cardServer `agent-card.ts:229-230`) → Instanz-Bindung ist **neue Verdrahtung**, nicht „eine schärfere Bedingung"; (3) ein **Broadcast-Wake ist strukturell nicht zustellbar** (weder `instance_id` noch `spiffe_uri` zum Matchen) und berührt den in **#277** geschlossenen Leak **D1** — bei fehlendem Anwendungsfall bleibt er zu Recht liegen; (4) der **Sweep** ist mechanisch billig (`AgentInbox.unreadCount({forInstance})` `agent-inbox.ts:468` + Index `:187` + `agentRegistry.list()` — **keine** neue Speicherarbeit), aber **vertraglich teuer**: Kernfrage ist die **Coalescer-Interaktion** (Sweep-Wake geschluckt ⇒ verpufft genau im Reconnect-Fenster, das er beheben soll; Coalescer umgangen ⇒ die §5-Zusage „≤1 Wake pro Instanz pro Fenster" ist **nicht mehr wörtlich wahr** und die Consumer-Spec müsste nachgezogen werden), zudem verschöbe er eine **Konsumenten-Pflicht** (Spec §5 + Runbook) zum Daemon. **Empfehlung (nicht entschieden):** Sweep zuerst entscheiden, Instanz-Bindung bewusst budgetieren, Broadcast liegen lassen; je Punkt ist benannt, welcher **reine** Folge-Slice danach ungegatet baubar wäre. **Alle Zeilenangaben gegen `a1ae1c2` verifiziert.** **CR:** externes Review am PR (Fokus: Zitat-Korrektheit + ob „entscheidungsgebunden" ehrlich ist oder ein baubarer Slice weggeredet wird). **PC:** Secret-Scan clean (nur Doku). **Unverändert gated und nicht angefasst:** TL-11 Slice B (out-of-repo Hop), TL-12 B/C (owner/CO), TL-10-Verdrahtung (D1-Loader/D3) — kein Loader, kein Flag-Flip, kein Host-Hop. Kein Code/Test/State/Deploy/Secret, Risiko-Delta **null**. |

**Typ:** Doc-only Scoping/Discovery (ADR-047 Proposed — entscheidet nichts, benennt die Entscheidungen). Kein Deploy/Secret. **DO:** `docs/architecture/ADR-047-tl11-wake-followups-scoping.md`, `TODO.md` (TL-11-Querverweis), `changes/2026-07-23_adr-047-tl11-followups-scoping.md`, `CHANGES.md`, dieser Eintrag.

---

*Letzte Aktualisierung: 2026-07-23 13:25 — docs(arch): ADR-047 TL-11-Wake-Nachlauf-Scoping; alle drei „Optional/danach"-Punkte sind entscheidungsgebunden (kein ungegateter Code-Slice), Befunde: `agentFilter`-Doppelfunktion, fehlende Client-Cert-Identität im WS-Handler, Broadcast strukturell nicht routbar (D1), Sweep billig aber Coalescer-Vertragsfrage; Empfehlung Sweep → Bindung → Broadcast; doc-only, Zeilenangaben gegen a1ae1c2 verifiziert.*
