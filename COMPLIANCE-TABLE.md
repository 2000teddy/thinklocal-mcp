...
*Letzte Aktualisierung: 2026-07-23 12:45 — test(tl11): Emitter-Ende-zu-Ende-Conformance (`inbox:new` → echter `/ws`-Socket); schließt die Naht zwischen Emitter und WS-Routing (bisherige Draht-Tests injizierten `agent:wake` direkt), +8 Tests inkl. Coalescing/Fail-closed/kein-Broadcast, mutations-verifiziert; §7.1-`it.todo`-Drift auf den #283-Stand korrigiert; Suite 1979 grün; test-only, Slice B bleibt extern-gated.*
