...
| 110 | tbd       | ADR-006 Phase 1 — Agent Session Persistence & Crash Recovery MVP | 04-09 18:51 | —  | —  | ✅ | ✅ | ✅ | ✅ | **Dieser PR.** Supersedes #85 (Design-only). 7 Module: atomic-write + session-events (SQLite WAL) + session-state + claude-code-adapter + recovery-generator (deterministisch) + session-watcher (poll-based mit per-instance Promise-Lock) + session-binding (orphan scan + fingerprint) + E2E Integration-Test. 53/53 Tests gruen. CR Gemini Pro (security focus) — 0 CRITICAL, 2× HIGH (watcher race, isPidAlive PID-reuse), 2× MEDIUM (BOM, UNTRUSTED marker), 2× LOW (mutation, cleanup errors) — alle adressiert: Promise-Lock im Watcher, isPidAlive als known-limitation in ADR-006 dokumentiert, BOM strip + Regression-Test, UNTRUSTED Pflicht-Doku, IngestResult.newState immutable contract, Cleanup-Comment. PC Gemini Pro — 1× MEDIUM (State mutation anti-pattern) komplett entfernt: Watcher liest jetzt `state.json` unter dem Lock neu als authoritative source. CO entfaellt (Design-Konsensus aus PR #85). |
